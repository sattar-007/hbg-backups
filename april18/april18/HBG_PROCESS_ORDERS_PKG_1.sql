...
----------------------------------------------------------------------------------------------------------------------------------------
	 ...
	 PROCEDURE generate_fbdi (
        ...
        l_payment_tkn_file         	utl_file.file_type;
        l_payment_tkn_file_name		VARCHAR2(100) := 'DooOrderPaymentsInt.csv';
        l_payment_tkn_blob          BLOB;
        l_charges_file         	    utl_file.file_type;
        l_charges_file_name		    VARCHAR2(100) := 'DooOrderChargesInt.csv';
        l_charges_blob              BLOB;
        l_charges_comps_file        utl_file.file_type;
        l_charges_comps_file_name   VARCHAR2(100) := 'DooOrderChargeCompsInt.csv';
        l_charges_comps_blob        BLOB;
        I_USER VARCHAR2(200);
        I_HOST VARCHAR2(200);
        I_PORT NUMBER;
    ...
    BEGIN
        ...
        IF p_source_system = 'STERLING' THEN
          ftp_filename := 'HBG_Sterling_Orders_' ||p_SBI_UUID||'.zip';
        ELSIF p_source_system = 'CX' THEN
          ftp_filename := 'HBG_CX_Orders_' ||p_SBI_UUID||'.zip'; 
        END IF;
	 /*-----------------------------------------------------------------------------------------------------
    ...
    /*-----------------------------------------------------------------------------------------------------
			GENERATE FBDI PAYMENT TOKEN CSV FILE
	------------------------------------------------------------------------------------------------------*/
        l_payment_tkn_file := utl_file.fopen(g_directory, l_payment_tkn_file_name, 'W', 32767);

		/*----------------------------------------------------------------------
				FOR EACH RECORD IN HBG_PROCESS_ORDERS_PAYMENT_TKN_FBDI TABLE
		------------------------------------------------------------------------*/
        ...
        FOR l_rec IN (
            SELECT
                *
            FROM
                HBG_PROCESS_ORDERS_PAYMENT_TKN_FBDI
            WHERE
                SBI_UUID = p_SBI_UUID
			and status = 'VALIDATED'
        ) LOOP
			...
			/*----------------------------------------------------------------------------
					WRITE ADRESSES ROW IN CSV FILE
			------------------------------------------------------------------------------*/
            utl_file.put_line(l_payment_tkn_file, '"'||

                                        l_rec.SOURCE_TRANSACTION_SYSTEM      		|| '","' ||								
                                        l_rec.SOURCE_TRANSACTION_ID          		|| '","' ||								
                                        l_rec.SOURCE_TRANSACTION_LINE_ID     		|| '","' ||								
                                        l_rec.SOURCE_TRANSACTION_SCHEDULE_ID 		|| '","' ||								
                                        l_rec.PAYMENT_METHOD_CODE            		|| '","' ||								
                                        l_rec.PAYMENT_METHOD                 		|| '","' ||								
                                        l_rec.PAYMENT_TRANSACTION_ID         		|| '","' ||								
                                        l_rec.PAYMENT_SET_ID                 		|| '","' ||								
                                        l_rec.SOURCE_TRANSACTION_PAYMENT_ID  		|| '","' ||								
                                        l_rec.PAYMENT_TYPE                   		|| '","' ||								
                                        l_rec.PAYMENT_TYPE_CODE              		|| '","' ||								
                                        l_rec.CC_TOKEN_NUMBER                		|| '","' ||								
                                        l_rec.CC_EXPIRATION_DATE             		|| '","' ||								
                                        l_rec.CC_FIRST_NAME                  		|| '","' ||								
                                        l_rec.CC_LAST_NAME                   		|| '","' ||								
                                        l_rec.CC_ISSUER_CODE                 		|| '","' ||								
                                        l_rec.CC_MASKED_NUMBER               		|| '","' ||								
                                        l_rec.CC_AUTH_REQUEST_ID            		|| '","' ||								
                                        l_rec.CC_VOICE_AUTH_CODE            		|| '","' ||								
                                        l_rec.PAYMENT_SERVER_ORDER_NUM      		|| '","' ||								
                                        l_rec.AUTHORIZED_AMOUNT             	    || '"' );

        END LOOP;

		/*----------------------------------------------------------------------
					CLOSE AND ADD PAYMENT TOKEN FILE TO ZIP 
		------------------------------------------------------------------------*/

        utl_file.fclose(l_payment_tkn_file);
        l_payment_tkn_blob := file_to_blob(l_payment_tkn_file_name);
        apex_zip.add_file(p_zipped_blob => x_zip_header, p_file_name => l_payment_tkn_file_name, p_content => l_payment_tkn_blob);

    /*-----------------------------------------------------------------------------------------------------
			GENERATE FBDI CHARGES CSV FILE
	------------------------------------------------------------------------------------------------------*/
        l_charges_file := utl_file.fopen(g_directory, l_charges_file_name, 'W', 32767);

		/*----------------------------------------------------------------------
				FOR EACH RECORD IN HBG_PROCESS_ORDERS_CHARGES_FBDI TABLE
		------------------------------------------------------------------------*/
        ...
        FOR l_rec IN (
            SELECT
                *
            FROM
                HBG_PROCESS_ORDERS_CHARGES_FBDI
            WHERE
                SBI_UUID = p_SBI_UUID
			and status = 'VALIDATED'
        ) LOOP
			...
			/*----------------------------------------------------------------------------
					WRITE CHARGES ROW IN CSV FILE
			------------------------------------------------------------------------------*/
                utl_file.put_line(l_charges_file, '"' ||
                    l_rec.SOURCE_TRANSACTION_ID          	|| '","' ||
                    l_rec.SOURCE_TRANSACTION_SYSTEM      	|| '","' ||
                    l_rec.SOURCE_TRANSACTION_LINE_ID      	|| '","' ||
                    l_rec.SOURCE_TRANSACTION_SCHEDULE_ID 	|| '","' ||
                    l_rec.SOURCE_CHARGE_ID               	|| '","' ||
                    l_rec.CHARGE_DEFINITION_CODE         	|| '","' ||
                    l_rec.CHARGE_DEFINITION              	|| '","' ||
                    l_rec.CHARGE_TYPE_CODE               	|| '","' ||
                    l_rec.CHARGE_TYPE                    	|| '","' ||
                    l_rec.CHARGE_SUBTYPE_CODE            	|| '","' ||
                    l_rec.CHARGE_SUBTYPE                 	|| '","' ||
                    l_rec.SEQUENCE_NUMBER                	|| '","' ||
                    l_rec.PRICE_TYPE_CODE                	|| '","' ||
                    l_rec.PRICE_TYPE			   			|| '","' ||
                    l_rec.PRICED_QUANTITY                	|| '","' ||
                    l_rec.PRICED_QUANTITY_UOM_CODE       	|| '","' ||
                    l_rec.PRICED_QUANTITY_UOM		    	|| '","' ||
                    l_rec.PRIMARY_FLAG                   	|| '","' ||
                    l_rec.APPLY_TO                       	|| '","' ||
                    l_rec.ROLLUP_FLAG                    	|| '","' ||
                    l_rec.CHARGE_CURRENCY_CODE           	|| '","' ||
                    l_rec.CHARGE_CURRENCY_NAME		    	|| '","' ||
                    l_rec.PRICE_PERIODICITY_CODE         	|| '","' ||
                    l_rec.GSA_UNIT_PRICE                 	|| '","' ||
                    l_rec.TRANSACTIONAL_CURRENCY_CODE    	|| '","' ||
                    l_rec.TRANSACTIONAL_UOM_CODE         	|| '","' ||
                    l_rec.AVERAGE_UNIT_SELLING_PRICE	   	|| '","' ||
                    l_rec.TIER_APPLIES_TO                	|| '","' ||
                    l_rec.TIER_APPLIES_TO_NAME           	|| '","' ||
                    l_rec.BLOCK_ALLOWANCE                	|| '","' ||
                    l_rec.BLOCK_SIZE			   			|| '"');				

        END LOOP;

		/*----------------------------------------------------------------------
					CLOSE AND ADD CHARGES FILE TO ZIP 
		------------------------------------------------------------------------*/

        utl_file.fclose(l_charges_file);
        l_charges_blob := file_to_blob(l_charges_file_name);
        apex_zip.add_file(p_zipped_blob => x_zip_header, p_file_name => l_charges_file_name, p_content => l_charges_blob);   
        
    /*-----------------------------------------------------------------------------------------------------
			GENERATE FBDI CHARGE COMPS CSV FILE
	------------------------------------------------------------------------------------------------------*/
        l_charges_comps_file := utl_file.fopen(g_directory, l_charges_comps_file_name, 'W', 32767);

		/*----------------------------------------------------------------------
				FOR EACH RECORD IN HBG_PROCESS_ORDERS_CHARGES_COMPS_FBDI TABLE
		------------------------------------------------------------------------*/
        ...
        FOR l_rec IN (
            SELECT
                *
            FROM
                HBG_PROCESS_ORDERS_CHARGES_COMPS_FBDI
            WHERE
                SBI_UUID = p_SBI_UUID
			and status = 'VALIDATED'
        ) LOOP
			...
			/*----------------------------------------------------------------------------
					WRITE CHARGES ROW IN CSV FILE
			------------------------------------------------------------------------------*/
                utl_file.put_line(l_charges_comps_file, '"' ||
                    l_rec.SOURCE_TRANSACTION_ID          	|| '","' ||
                    l_rec.SOURCE_TRANSACTION_SYSTEM      	|| '","' ||
                    l_rec.SOURCE_TRANSACTION_LINE_ID      	|| '","' ||
                    l_rec.SOURCE_TRANSACTION_SCHEDULE_ID 	|| '","' ||
                    l_rec.SOURCE_CHARGE_COMPONENT_ID     	|| '","' ||
                    l_rec.SOURCE_CHARGE_ID               	|| '","' ||
                    l_rec.SEQUENCE_NUMBER                	|| '","' ||
                    l_rec.SOURCE_PARENT_CHARGE_COMP_ID   	|| '","' ||
                    l_rec.CHARGE_CURRENCY_CODE           	|| '","' ||
                    l_rec.CHARGE_CURRENCY_NAME 	   		 	|| '","' ||
                    l_rec.CHARGE_CURRENCY_EXT_AMOUNT     	|| '","' ||
                    l_rec.CHARGE_CURRENCY_UNIT_PRICE     	|| '","' ||
                    l_rec.HEADER_CURRENCY_CODE           	|| '","' ||
                    l_rec.HEADER_CURRENCY_NAME		   		|| '","' ||
                    l_rec.HEADER_CURRENCY_EXT_AMOUNT     	|| '","' ||
                    l_rec.HEADER_CURRENCY_UNIT_PRICE     	|| '","' ||
                    l_rec.PRICE_ELEMENT_CODE            	|| '","' ||
                    l_rec.PRICE_ELEMENT		   		    	|| '","' ||
                    l_rec.PRICE_ELEMENT_USAGE_CODE       	|| '","' ||
                    l_rec.PRICE_ELEMENT_USAGE		    	|| '","' ||
                    l_rec.ROLLUP_FLAG                    	|| '","' ||
                    l_rec.TRANSACTIONAL_CURRENCY_CODE   	|| '","' ||
                    l_rec.EXPLANATION                    	|| '","' ||
                    l_rec.SOURCE_MPA_ID				    	|| '","' ||
                    l_rec.CHARGE_CURR_DURATION_EXT_AMT   	|| '","' ||
                    l_rec.HEADER_CURR_DURATION_EXT_AMT   	|| '"');				

        END LOOP;

		/*----------------------------------------------------------------------
					CLOSE AND ADD CHARGES COMPS FILE TO ZIP 
		------------------------------------------------------------------------*/

        utl_file.fclose(l_charges_comps_file);
        l_charges_comps_blob := file_to_blob(l_charges_comps_file_name);
        apex_zip.add_file(p_zipped_blob => x_zip_header, p_file_name => l_charges_comps_file_name, p_content => l_charges_comps_blob);    
...
-----------------------------------------------------------------------------------------------------------------------------------------------
    ...
    --DATA FOR SALES ORDER PAYMENT TOKEN FBDI
	CURSOR c_doo_payment_tkn_all_interface IS
		SELECT 
				LPAD(GIS_HEADER_UID,p_headers_count,'0') AS GIS_HEADER_UID,
                hdr.PURCHASE_ORDER_NO,
                CASE WHEN regexp_substr(PAYMENT_TOKEN, '[^;]+', 1, 5) IS NOT NULL THEN regexp_substr(PAYMENT_TOKEN, '[^;]+', 1, 5)
                ELSE PAYMENT_TOKEN END AS PAYMENT_TOKEN
		FROM 	HBG_STERLING_HEADER hdr
		WHERE 	hdr.SBI_UUID = p_sbi_uuid
			AND hdr.IMPORT_STATUS = 'NEW'
            and PAYMENT_TOKEN is not null;

    --DATA FOR SALES ORDER COMMENTS/ATTACHMENTS
    CURSOR c_doo_attachments_interface  IS
        SELECT
            LPAD(GIS_HEADER_UID,p_headers_count,'0') AS GIS_HEADER_UID,
            PURCHASE_ORDER_NO,
            COMMENT_TEXT1 AS COMMENT_TEXT,
            COMMENT_TYPE1 AS COMMENT_TYPE,
            1             AS COMMENT_NO
        FROM HBG_STERLING_COMMENT cmt
        WHERE 	cmt.SBI_UUID = p_sbi_uuid
			AND cmt.IMPORT_STATUS = 'NEW'
            AND COMMENT_TEXT1 IS NOT NULL
        UNION
        SELECT
            LPAD(GIS_HEADER_UID,p_headers_count,'0') AS GIS_HEADER_UID,
            PURCHASE_ORDER_NO,
            COMMENT_TEXT2 AS COMMENT_TEXT,
            COMMENT_TYPE2 AS COMMENT_TYPE,
            2             AS COMMENT_NO
        FROM HBG_STERLING_COMMENT cmt
        WHERE 	cmt.SBI_UUID = p_sbi_uuid
			AND cmt.IMPORT_STATUS = 'NEW'
            AND COMMENT_TEXT2 IS NOT NULL
        UNION
        SELECT
            LPAD(GIS_HEADER_UID,p_headers_count,'0') AS GIS_HEADER_UID,
            PURCHASE_ORDER_NO,
            COMMENT_TEXT3 AS COMMENT_TEXT,
            COMMENT_TYPE3 AS COMMENT_TYPE,
            3             AS COMMENT_NO
        FROM HBG_STERLING_COMMENT cmt
        WHERE 	cmt.SBI_UUID = p_sbi_uuid
			AND cmt.IMPORT_STATUS = 'NEW'
            AND COMMENT_TEXT3 IS NOT NULL
        UNION
        SELECT
            LPAD(GIS_HEADER_UID,p_headers_count,'0') AS GIS_HEADER_UID,
            PURCHASE_ORDER_NO,
            COMMENT_TEXT4 AS COMMENT_TEXT,
            COMMENT_TYPE4 AS COMMENT_TYPE,
            4             AS COMMENT_NO
        FROM HBG_STERLING_COMMENT cmt
        WHERE 	cmt.SBI_UUID = p_sbi_uuid
			AND cmt.IMPORT_STATUS = 'NEW'
            AND COMMENT_TEXT4 IS NOT NULL
        UNION
        SELECT
            LPAD(GIS_HEADER_UID,p_headers_count,'0') AS GIS_HEADER_UID,
            PURCHASE_ORDER_NO,
            COMMENT_TEXT5 AS COMMENT_TEXT,
            COMMENT_TYPE5 AS COMMENT_TYPE,
            5             AS COMMENT_NO
        FROM HBG_STERLING_COMMENT cmt
        WHERE 	cmt.SBI_UUID = p_sbi_uuid
			AND cmt.IMPORT_STATUS = 'NEW'
            AND COMMENT_TEXT5 IS NOT NULL
        UNION
        SELECT
            LPAD(GIS_HEADER_UID,p_headers_count,'0') AS GIS_HEADER_UID,
            PURCHASE_ORDER_NO,
            COMMENT_TEXT6 AS COMMENT_TEXT,
            COMMENT_TYPE6 AS COMMENT_TYPE,
            6             AS COMMENT_NO
        FROM HBG_STERLING_COMMENT cmt
        WHERE 	cmt.SBI_UUID = p_sbi_uuid
			AND cmt.IMPORT_STATUS = 'NEW'
            AND COMMENT_TEXT6 IS NOT NULL
        UNION
        SELECT
            LPAD(GIS_HEADER_UID,p_headers_count,'0') AS GIS_HEADER_UID,
            PURCHASE_ORDER_NO,
            COMMENT_TEXT7 AS COMMENT_TEXT,
            COMMENT_TYPE7 AS COMMENT_TYPE,
            7             AS COMMENT_NO
        FROM HBG_STERLING_COMMENT cmt
        WHERE 	cmt.SBI_UUID = p_sbi_uuid
			AND cmt.IMPORT_STATUS = 'NEW'
            AND COMMENT_TEXT7 IS NOT NULL
        UNION
        SELECT
            LPAD(GIS_HEADER_UID,p_headers_count,'0') AS GIS_HEADER_UID,
            PURCHASE_ORDER_NO,
            COMMENT_TEXT8 AS COMMENT_TEXT,
            COMMENT_TYPE8 AS COMMENT_TYPE,
            8             AS COMMENT_NO
        FROM HBG_STERLING_COMMENT cmt
        WHERE 	cmt.SBI_UUID = p_sbi_uuid
			AND cmt.IMPORT_STATUS = 'NEW'
            AND COMMENT_TEXT8 IS NOT NULL
        UNION
        SELECT
            LPAD(GIS_HEADER_UID,p_headers_count,'0') AS GIS_HEADER_UID,
            PURCHASE_ORDER_NO,
            COMMENT_TEXT9 AS COMMENT_TEXT,
            COMMENT_TYPE9 AS COMMENT_TYPE,
            9             AS COMMENT_NO
        FROM HBG_STERLING_COMMENT cmt
        WHERE 	cmt.SBI_UUID = p_sbi_uuid
			AND cmt.IMPORT_STATUS = 'NEW'
            AND COMMENT_TEXT9 IS NOT NULL
        UNION
        SELECT
            LPAD(GIS_HEADER_UID,p_headers_count,'0') AS GIS_HEADER_UID,
            PURCHASE_ORDER_NO,
            COMMENT_TEXT10 AS COMMENT_TEXT,
            COMMENT_TYPE10 AS COMMENT_TYPE,
            10             AS COMMENT_NO
        FROM HBG_STERLING_COMMENT cmt
        WHERE 	cmt.SBI_UUID = p_sbi_uuid
			AND cmt.IMPORT_STATUS = 'NEW'
            AND COMMENT_TEXT10 IS NOT NULL
        ;
    ...
    TYPE lt_doo_payment_tkn_all_interface_tb  	IS TABLE OF c_doo_payment_tkn_all_interface%ROWTYPE;
    TYPE lt_doo_attachments_interface_tb  	    IS TABLE OF c_doo_attachments_interface%ROWTYPE;
	...
	l_doo_payment_tkn_all_interface_tb	lt_doo_payment_tkn_all_interface_tb;
    l_doo_attachments_interface_tb	    lt_doo_attachments_interface_tb;
	l_count									NUMBER := 0;
    ...
    execute immediate 'alter session set optimizer_index_cost_adj=10';
        ...
        dbms_output.put_line(systimestamp); 
            ...
            -------------------------------------------------------------------------------------
				BEGIN
					FORALL i IN 1 .. l_doo_header_all_interface_tb.COUNT SAVE EXCEPTIONS
						...
						(
							...
							'HBG_INTEGRATION',                                                  --CREATED_BY,
                            'HBG_INTEGRATION'                                                   --LAST_UPDATED_BY,
						);
				...
				EXCEPTION 
                    ...
                    dbms_output.put_line('HEADER_EXCEPTION'); 
					/*-----------------------------------------------------------------------------
		...
		/*-----------------------------------------------------------------------------------------------------
			BEGIN PROCESS PAYMENT TOKEN DATA
		------------------------------------------------------------------------------------------------------*/
        l_count				:= 0;
		l_count_success		:= 0;
		l_count_fail		:= 0;
		...
		BEGIN
			...
			IF p_debug = 'true' then
				DEBUG_MSG ( 
					p_file_name => p_debug_filename,
					p_debug_msg => 'BEGIN PROCESS PAYMENT TOKEN DATA - OPEN c_doo_payment_tkn_all_interface' );
			end if;
            dbms_output.put_line('TOKEN');
			OPEN c_doo_payment_tkn_all_interface;
			LOOP
				FETCH c_doo_payment_tkn_all_interface BULK COLLECT INTO l_doo_payment_tkn_all_interface_tb LIMIT 1000;
				EXIT WHEN l_doo_payment_tkn_all_interface_tb.COUNT = 0;
                
				l_count := l_count + l_doo_payment_tkn_all_interface_tb.COUNT;
				IF p_debug = 'true' then
					DEBUG_MSG ( 
						p_file_name => p_debug_filename,
						p_debug_msg => 'BEGIN PROCESS PAYMENT TOKEN DATA - Bulk Collect count '||l_count );
				end if;
            ------------------------------------------------------------------------------------
            /*              MAPPING FOR PAYMENT TOKENS                                         */
            -------------------------------------------------------------------------------------
                BEGIN
					FORALL i IN 1 .. l_doo_payment_tkn_all_interface_tb.count SAVE EXCEPTIONS
						INSERT INTO HBG_PROCESS_ORDERS_PAYMENT_TKN_FBDI
						(
							SOURCE_TRANSACTION_ID, 	
							SOURCE_TRANSACTION_SYSTEM,
							PAYMENT_METHOD,
							SOURCE_TRANSACTION_PAYMENT_ID,			
							PAYMENT_TYPE_CODE,
                            CC_TOKEN_NUMBER,
							GIS_ID,					
							SBI_UUID,                   
						...
						VALUES
						(
							p_sbi_uuid || l_doo_payment_tkn_all_interface_tb(i).GIS_HEADER_UID,	--SOURCE_TRANSACTION_ID, 	
							l_source_transaction_sys,                                           --SOURCE_TRANSACTION_SYSTEM,
							'Credit Card',                                                      --PAYMENT_METHOD,
							p_sbi_uuid || l_doo_payment_tkn_all_interface_tb(i).GIS_HEADER_UID, --SOURCE_TRANSACTION_PAYMENT_ID,
							'ORA_CREDIT_CARD',                                                  --PAYMENT_TYPE_CODE,
                            l_doo_payment_tkn_all_interface_tb(i).PAYMENT_TOKEN,                --CC_TOKEN_NUMBER,
							p_gid_id,                                                           --GIS_ID,					
							p_sbi_uuid,                                                         --SBI_UUID,                   
							'NEW',                                                              --STATUS,					         
							'STERLING',                                                         --SOURCE_SYSTEM,              
					...
					l_count_success := l_count_success + l_doo_payment_tkn_all_interface_tb.COUNT;
				EXCEPTION 

					WHEN le_insert_exception THEN
                    dbms_output.put_line('PAYMENT_TOKEN_EXCEPTION'); 
					/*-----------------------------------------------------------------------------
						UPDATE PROCESS TABLES WITH HEADER ERROR MSG
					-------------------------------------------------------------------------------*/
						FOR i IN 1 .. SQL%BULK_EXCEPTIONS.COUNT LOOP
						l_errbuf := 'ERROR INSERTING PAYMENT TOKEN - ' || SQLERRM(-SQL%BULK_EXCEPTIONS(i).ERROR_CODE);
                        --dbms_output.put_line('l_errbuf - ' || l_errbuf); 
						l_update_errmsg := UPDATE_ERROR_MSG	(	l_errbuf, 
																l_doo_payment_tkn_all_interface_tb(i).PURCHASE_ORDER_NO, 
																p_sbi_uuid || l_doo_payment_tkn_all_interface_tb(i).GIS_HEADER_UID, 
																p_sbi_uuid, 
																'NEW',
                                                                'STERLING');
                                                                
                        --dbms_output.put_line('l_update_errmsg - ' || l_update_errmsg); 
						l_retcode := 1;
						COMMIT;
						...
						l_count_fail := l_count_fail + SQL%BULK_EXCEPTIONS.COUNT;
						l_count_success := l_count_success + l_doo_payment_tkn_all_interface_tb.COUNT - SQL%BULK_EXCEPTIONS.COUNT;
            ...
            IF p_debug = 'true' then
                DEBUG_MSG ( 
                    p_file_name => p_debug_filename,
                    p_debug_msg => 'BEGIN PROCESS PAYMENT TOKEN DATA - '||l_count_fail||' orders failed');
            end if;

            IF p_debug = 'true' then
                DEBUG_MSG ( 
                    p_file_name => p_debug_filename,
                    p_debug_msg => 'BEGIN PROCESS PAYMENT TOKEN DATA - '||l_count_success||' orders succeeded');
            end if;
			CLOSE c_doo_payment_tkn_all_interface;

			IF p_debug = 'true' then
                DEBUG_MSG ( 
                            p_file_name => p_debug_filename,
                            p_debug_msg => 'end of c_doo_payment_tkn_all_interface loop' );
            end if;

		EXCEPTION 
			WHEN OTHERS THEN
				IF c_doo_payment_tkn_all_interface%ISOPEN THEN
				   CLOSE c_doo_payment_tkn_all_interface;
				END IF;
                dbms_output.put_line(sqlerrm);
				l_errbuf := sqlerrm;
		...
		BEGIN
			...
			IF p_debug = 'true' then
				DEBUG_MSG ( 
					p_file_name => p_debug_filename,
					p_debug_msg => 'BEGIN PROCESS COMMENTS DATA - OPEN c_doo_attachments_interface' );
			end if;
            dbms_output.put_line('TOKEN');
			OPEN c_doo_attachments_interface;
			LOOP
				FETCH c_doo_attachments_interface BULK COLLECT INTO l_doo_attachments_interface_tb LIMIT 1000;
				EXIT WHEN l_doo_attachments_interface_tb.COUNT = 0;
                
				l_count := l_count + l_doo_attachments_interface_tb.COUNT;
				IF p_debug = 'true' then
					DEBUG_MSG ( 
						p_file_name => p_debug_filename,
						p_debug_msg => 'BEGIN PROCESS COMMENTS DATA - Bulk Collect count '||l_count );
				end if;
            ------------------------------------------------------------------------------------
            /*              MAPPING FOR PAYMENT TOKENS                                         */
            -------------------------------------------------------------------------------------
                BEGIN
					FORALL i IN 1 .. l_doo_attachments_interface_tb.count SAVE EXCEPTIONS
						INSERT INTO HBG_PROCESS_ORDERS_COMMENTS
						(
							SOURCE_TRANSACTION_ID, 	
							SOURCE_TRANSACTION_SYSTEM,
							DATA_TYPE_CODE,
						...
						VALUES
						(
							p_sbi_uuid || l_doo_attachments_interface_tb(i).GIS_HEADER_UID,	    --SOURCE_TRANSACTION_ID, 	
							l_source_transaction_sys,                                           --SOURCE_TRANSACTION_SYSTEM,
							'TEXT',                                                             --DATA_TYPE_CODE,
							'GEN',                                                              --CATEGORY_NAME,
							'text/plain',                                                       --CONTENT_TYPE,			
							l_doo_attachments_interface_tb(i).COMMENT_TEXT,                     --UPLOADED_TEXT,
                            l_doo_attachments_interface_tb(i).COMMENT_NO,                       --COMMENT_NO
							p_sbi_uuid,                                                         --BATCH_ID,                   
							'NEW',                                                              --STATUS,					         
							'STERLING',                                                         --SOURCE_SYSTEM,              
							sysdate,                                                            --CREATION_DATE, 
					...
					COMMIT;
                    dbms_output.put_line('INSERT');
					l_count_success := l_count_success + l_doo_attachments_interface_tb.COUNT;
				EXCEPTION 

					WHEN le_insert_exception THEN
                    dbms_output.put_line('COMMENTS_EXCEPTION'); 
					/*-----------------------------------------------------------------------------
						UPDATE PROCESS TABLES WITH HEADER ERROR MSG
					-------------------------------------------------------------------------------*/
						FOR i IN 1 .. SQL%BULK_EXCEPTIONS.COUNT LOOP
						l_errbuf := 'ERROR INSERTING COMMETS - ' || SQLERRM(-SQL%BULK_EXCEPTIONS(i).ERROR_CODE);
						l_update_errmsg := UPDATE_ERROR_MSG	(	l_errbuf, 
																l_doo_attachments_interface_tb(i).PURCHASE_ORDER_NO, 
																p_sbi_uuid || l_doo_attachments_interface_tb(i).GIS_HEADER_UID, 
																p_sbi_uuid, 
																'NEW',
                                                                'STERLING');
						l_retcode := 1;
						...
						l_count_fail := l_count_fail + SQL%BULK_EXCEPTIONS.COUNT;
						l_count_success := l_count_success + l_doo_attachments_interface_tb.COUNT - SQL%BULK_EXCEPTIONS.COUNT;
            ...
            IF p_debug = 'true' then
                DEBUG_MSG ( 
                    p_file_name => p_debug_filename,
                    p_debug_msg => 'BEGIN PROCESS COMMENTS DATA - '||l_count_fail||' orders failed');
            end if;

            IF p_debug = 'true' then
                DEBUG_MSG ( 
                    p_file_name => p_debug_filename,
                    p_debug_msg => 'BEGIN PROCESS COMMENTS DATA - '||l_count_success||' orders succeeded');
            end if;
			CLOSE c_doo_attachments_interface;

			IF p_debug = 'true' then
                DEBUG_MSG ( 
                            p_file_name => p_debug_filename,
                            p_debug_msg => 'end of c_doo_attachments_interface loop' );
            end if;

		EXCEPTION 
			WHEN OTHERS THEN
				IF c_doo_attachments_interface%ISOPEN THEN
				   CLOSE c_doo_attachments_interface;
				END IF;
                dbms_output.put_line(sqlerrm);
				l_errbuf := sqlerrm;
		...
		BEGIN
			...
			LOOP
				...
				EXCEPTION 
					...
					-------------------------------------------------------------------------------*/
						FOR i IN 1 .. SQL%BULK_EXCEPTIONS.COUNT LOOP
                        dbms_output.put_line('LINE_EXCEPTION'); 
...
-----------------------------------------------------------------------------------------------------------------------------------------------
	...
	BEGIN 
		...
		errbuf := NULL;
		retcode := 0;
        --INSERT INTO HBG_PROCESS_ORDERS_DEBUG (TASK,TIME_STAMP) VALUES ('begin sterling to stage tables',systimestamp) ;
        --COMMIT;
		...
		-------------------------------------------------------------------------------------------------------*/	
            ...
            --INSERT INTO HBG_PROCESS_ORDERS_DEBUG (TASK,TIME_STAMP) VALUES ('Insert values in FBDI Tables - '||l_gisid_rec.SBI_UUID,systimestamp) ;
         --COMMIT;
...
-----------------------------------------------------------------------------------------------------------------------------------------------
    ...
    END IF;
        ...
        DELETE FROM HBG_PROCESS_ORDERS_PAYMENT_TKN_FBDI 
			WHERE 	SBI_UUID = p_SBI_UUID
			AND		(SOURCE_TRANSACTION_ID = p_transaction_id
						OR p_transaction_id IS NULL);
                        
        DELETE FROM HBG_PROCESS_ORDERS_CHARGES_FBDI 
			WHERE 	SBI_UUID = p_SBI_UUID
			AND		(SOURCE_TRANSACTION_ID = p_transaction_id
						OR p_transaction_id IS NULL);
        
        DELETE FROM HBG_PROCESS_ORDERS_CHARGES_COMPS_FBDI 
			WHERE 	SBI_UUID = p_SBI_UUID
			AND		(SOURCE_TRANSACTION_ID = p_transaction_id
						OR p_transaction_id IS NULL);
        
        DELETE FROM HBG_PROCESS_ORDERS_COMMENTS
			WHERE 	BATCH_ID = p_SBI_UUID
			AND		(SOURCE_TRANSACTION_ID = p_transaction_id
						OR p_transaction_id IS NULL);
...
------------------------------------------------------------------------------------------------------*/
    ...
    BEGIN
        ...
        errbuf := null;
        retcode := 0;
        --INSERT INTO HBG_PROCESS_ORDERS_DEBUG (TASK,TIME_STAMP) VALUES ('begin validate sterling data',systimestamp) ;
         --COMMIT;   
     -----------------------------------------------------------------------------------------------------------------------------------------------
    /*										Check for duplicated orders under the same Gis ID													    */
    -----------------------------------------------------------------------------------------------------------------------------------------------  
        --INSERT INTO HBG_PROCESS_ORDERS_DEBUG (TASK,TIME_STAMP) VALUES ('Check for duplicated orders under the same Gis Id',systimestamp) ;
        --COMMIT; 
    ...
    ------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        --INSERT INTO HBG_PROCESS_ORDERS_DEBUG (TASK,TIME_STAMP) VALUES ('REQUIRED FIELDS WITHIN HEADER',systimestamp) ;
        --COMMIT; 
        ...
        --INSERT INTO HBG_PROCESS_ORDERS_DEBUG (TASK,TIME_STAMP) VALUES ('REQUIRED FIELDS WITHIN LINE',systimestamp) ;
        --COMMIT; 
        dbms_output.put_line('check for lines required fields - ' || systimestamp);
        UPDATE HBG_PROCESS_ORDERS_LINES_FBDI 
            SET STATUS = 'ERROR',  
                COMMENTS = 'Missing Stage Account Number;'  
        WHERE SOURCE_ACCOUNT_NO IS NULL      
          AND SBI_UUID = '6d5b23a2-cfa4-47c3-869a-c3f9f8a12701';
    
         
         UPDATE HBG_PROCESS_ORDERS_LINES_FBDI 
            SET STATUS = 'ERROR',  
                COMMENTS = COMMENTS ||'Missing Purchase Order Number;'
            WHERE SOURCE_PURCHASE_ORDER_NO IS NULL      
              AND SBI_UUID = '6d5b23a2-cfa4-47c3-869a-c3f9f8a12701';
                                          
         UPDATE HBG_PROCESS_ORDERS_LINES_FBDI 
            SET STATUS = 'ERROR',  
                COMMENTS = COMMENTS || 'Missing GIS ID;'
            WHERE GIS_ID IS NULL      
              AND SBI_UUID = '6d5b23a2-cfa4-47c3-869a-c3f9f8a12701';
              
         UPDATE HBG_PROCESS_ORDERS_LINES_FBDI 
            SET STATUS = 'ERROR',  
                COMMENTS = COMMENTS ||'Missing GIS HEADER ID;'
            WHERE GIS_HEADER_ID IS NULL      
              AND SBI_UUID = '6d5b23a2-cfa4-47c3-869a-c3f9f8a12701';
              
       UPDATE HBG_PROCESS_ORDERS_LINES_FBDI 
            SET STATUS = 'ERROR',  
                COMMENTS = COMMENTS || 'Missing Product ID;'
            WHERE PRODUCT_NUMBER IS NULL      
              AND SBI_UUID = '6d5b23a2-cfa4-47c3-869a-c3f9f8a12701';
              
        UPDATE HBG_PROCESS_ORDERS_LINES_FBDI 
            SET STATUS = 'ERROR',  
                COMMENTS = COMMENTS ||'Missing Line Sequence;'
        ...
        --INSERT INTO HBG_PROCESS_ORDERS_DEBUG (TASK,TIME_STAMP) VALUES ('LINE QUANTITY VALIDATION',systimestamp) ;
        --COMMIT; 
        ...
        --REQUIRED FIELDS VALIDATION 2 - CHECK FOR VALID LINE OR VALID HEADER
         --INSERT INTO HBG_PROCESS_ORDERS_DEBUG (TASK,TIME_STAMP) VALUES ('CHECK FOR VALID LINE OR VALID HEADER',systimestamp) ;
        --COMMIT; 
    ...
    /*          HEADER GS1 TAGS                                                 */
    /*=========================================================================*/
    --INSERT INTO HBG_PROCESS_ORDERS_DEBUG (TASK,TIME_STAMP) VALUES ('HEADER GS1 TAGS  ',systimestamp) ;
        --COMMIT; 
    dbms_output.put_line('CHECK HEADER GS1 TAGS - ' || systimestamp);
 
        --GS1_LABEL_TAG1
        UPDATE HBG_PROCESS_ORDERS_HDRS_EFF_FBDI hdr
            SET STATUS = 'ERROR', 
                ERROR_MSG = 'GS1 code ['|| ATTRIBUTE_CHAR12 || '] does not exist on master data table;'
        WHERE   CONTEXT_CODE = 'GS1 Data'
            AND SBI_UUID = p_SBI_UUID
            AND ATTRIBUTE_CHAR12 IS NOT NULL
        ...
        UPDATE HBG_PROCESS_ORDERS_HDRS_EFF_FBDI hdr
            SET STATUS = 'ERROR', 
                ERROR_MSG = ERROR_MSG || 'GS1 code ['|| ATTRIBUTE_CHAR13 || '] does not exist on master data table;'
        WHERE   CONTEXT_CODE = 'GS1 Data'
            AND SBI_UUID = p_SBI_UUID
        ...
        --GS1_LABEL_TAG3    
         UPDATE HBG_PROCESS_ORDERS_HDRS_EFF_FBDI hdr
            SET STATUS = 'ERROR', 
                ERROR_MSG = ERROR_MSG || 'GS1 code ['|| ATTRIBUTE_CHAR14 || '] does not exist on master data table;'
        WHERE   CONTEXT_CODE = 'GS1 Data'
            AND SBI_UUID = p_SBI_UUID
            AND ATTRIBUTE_CHAR14 IS NOT NULL
            AND NOT EXISTS (SELECT 1 FROM HBG_STERLING_GS1_DATA_TAGS WHERE CODE = hdr.ATTRIBUTE_CHAR14 );

        --GS1_LABEL_TAG4    
        UPDATE HBG_PROCESS_ORDERS_HDRS_EFF_FBDI hdr
            SET STATUS = 'ERROR', 
                ERROR_MSG = ERROR_MSG || 'GS1 code ['|| ATTRIBUTE_CHAR15 || '] does not exist on master data table;'
        WHERE   CONTEXT_CODE = 'GS1 Data'
            AND SBI_UUID = p_SBI_UUID
            AND ATTRIBUTE_CHAR15 IS NOT NULL
            AND NOT EXISTS (SELECT 1 FROM HBG_STERLING_GS1_DATA_TAGS WHERE CODE = hdr.ATTRIBUTE_CHAR15 );
        
        --GS1_LABEL_TAG5    
         UPDATE HBG_PROCESS_ORDERS_HDRS_EFF_FBDI hdr
            SET STATUS = 'ERROR', 
                ERROR_MSG = ERROR_MSG || 'GS1 code ['|| ATTRIBUTE_CHAR16 || '] does not exist on master data table;'
        WHERE   CONTEXT_CODE = 'GS1 Data'
            AND SBI_UUID = p_SBI_UUID
            AND ATTRIBUTE_CHAR16 IS NOT NULL
            AND NOT EXISTS (SELECT 1 FROM HBG_STERLING_GS1_DATA_TAGS WHERE CODE = hdr.ATTRIBUTE_CHAR16 );

        --GS1_LABEL_TAG6
         UPDATE HBG_PROCESS_ORDERS_HDRS_EFF_FBDI hdr
            SET STATUS = 'ERROR', 
                ERROR_MSG = ERROR_MSG || 'GS1 code ['|| ATTRIBUTE_CHAR17 || '] does not exist on master data table;'
        WHERE   CONTEXT_CODE = 'GS1 Data'
            AND SBI_UUID = p_SBI_UUID
            AND ATTRIBUTE_CHAR17 IS NOT NULL
            AND NOT EXISTS (SELECT 1 FROM HBG_STERLING_GS1_DATA_TAGS WHERE CODE = hdr.ATTRIBUTE_CHAR17 );
        
     
        --GS1_LABEL_TAG7
         UPDATE HBG_PROCESS_ORDERS_HDRS_EFF_FBDI hdr
            SET STATUS = 'ERROR', 
                ERROR_MSG = ERROR_MSG || 'GS1 code ['|| ATTRIBUTE_CHAR18 || '] does not exist on master data table;'
        WHERE   CONTEXT_CODE = 'GS1 Data'
            AND SBI_UUID = p_SBI_UUID
            AND ATTRIBUTE_CHAR18 IS NOT NULL
            AND NOT EXISTS (SELECT 1 FROM HBG_STERLING_GS1_DATA_TAGS WHERE CODE = hdr.ATTRIBUTE_CHAR18 );
        

        --GS1_LABEL_TAG8
         UPDATE HBG_PROCESS_ORDERS_HDRS_EFF_FBDI hdr
            SET STATUS = 'ERROR', 
                ERROR_MSG = ERROR_MSG || 'GS1 code ['|| ATTRIBUTE_CHAR19 || '] does not exist on master data table;'
        WHERE   CONTEXT_CODE = 'GS1 Data'
            AND SBI_UUID = p_SBI_UUID
            AND ATTRIBUTE_CHAR19 IS NOT NULL
            AND NOT EXISTS (SELECT 1 FROM HBG_STERLING_GS1_DATA_TAGS WHERE CODE = hdr.ATTRIBUTE_CHAR19 );
            
        --GS1_LABEL_TAG9
        UPDATE HBG_PROCESS_ORDERS_HDRS_EFF_FBDI hdr
            SET STATUS = 'ERROR', 
                ERROR_MSG = ERROR_MSG || 'GS1 code ['|| ATTRIBUTE_CHAR20 || '] does not exist on master data table;'
        WHERE   CONTEXT_CODE = 'GS1 Data'
            AND SBI_UUID = p_SBI_UUID
            AND ATTRIBUTE_CHAR20 IS NOT NULL
            AND NOT EXISTS (SELECT 1 FROM HBG_STERLING_GS1_DATA_TAGS WHERE CODE = hdr.ATTRIBUTE_CHAR20 );
            
        --GS1_LABEL_TAG10
         UPDATE HBG_PROCESS_ORDERS_HDRS_EFF_FBDI hdr
            SET STATUS = 'ERROR', 
                ERROR_MSG = ERROR_MSG || 'GS1 code ['|| ATTRIBUTE_CHAR1 || '] does not exist on master data table;'
        WHERE   CONTEXT_CODE = 'GS1 Data'
            AND SBI_UUID = p_SBI_UUID
            AND ATTRIBUTE_CHAR1 IS NOT NULL
            AND NOT EXISTS (SELECT 1 FROM HBG_STERLING_GS1_DATA_TAGS WHERE CODE = hdr.ATTRIBUTE_CHAR1 );
    ...
    /*          LINE GS1 TAGS                                                 */
    /*=========================================================================*/
    --INSERT INTO HBG_PROCESS_ORDERS_DEBUG (TASK,TIME_STAMP) VALUES ('LINE GS1 TAGS   ',systimestamp) ; 
    --COMMIT;
    dbms_output.put_line('CHECK LINES GS1 TAGS - ' || systimestamp);
 
 
        --GS1_LABEL_TAG1
        UPDATE HBG_PROCESS_ORDERS_LINES_EFF_FBDI hdr
            SET STATUS = 'ERROR', 
                ERROR_MSG = 'GS1 code ['|| ATTRIBUTE_CHAR11 || '] does not exist on master data table;'
        WHERE   CONTEXT_CODE = 'GS1 Data'
            AND SBI_UUID = p_SBI_UUID
            AND ATTRIBUTE_CHAR11 IS NOT NULL
            AND NOT EXISTS (SELECT 1 FROM HBG_STERLING_GS1_DATA_TAGS WHERE CODE = hdr.ATTRIBUTE_CHAR11 );
           
        --GS1_LABEL_TAG2  
       UPDATE HBG_PROCESS_ORDERS_LINES_EFF_FBDI hdr
            SET STATUS = 'ERROR', 
                ERROR_MSG = ERROR_MSG || 'GS1 code ['|| ATTRIBUTE_CHAR12 || '] does not exist on master data table;'
        WHERE   CONTEXT_CODE = 'GS1 Data'
            AND SBI_UUID = p_SBI_UUID
            AND ATTRIBUTE_CHAR12 IS NOT NULL
            AND NOT EXISTS (SELECT 1 FROM HBG_STERLING_GS1_DATA_TAGS WHERE CODE = hdr.ATTRIBUTE_CHAR12 );
    ...
    ----------------------------------------------------------------------------------------------------------------------------------------------------------
           --INSERT INTO HBG_PROCESS_ORDERS_DEBUG (TASK,TIME_STAMP) VALUES ('SEND ERROR STATUS BETWEEN TABLES',systimestamp) ; 
            --COMMIT;
            --UPDATE LINES TABLE
            ...
            --UPDATE PAYMENT TOKEN TABLE
            dbms_output.put_line('PAYMENT TOKEN ERROR UPDATE - ' || systimestamp);
            UPDATE HBG_PROCESS_ORDERS_PAYMENT_TKN_FBDI tkn
            SET STATUS = 'ERROR'
            WHERE EXISTS(SELECT hdr.source_transaction_id FROM HBG_PROCESS_ORDERS_HEADERS_FBDI hdr WHERE HDR.source_transaction_id = tkn.source_transaction_id and 
                            hdr.STATUS = 'ERROR' )
            AND SBI_UUID = p_SBI_UUID
            AND status = 'NEW'; 
            
            --UPDATE COMMENTS TABLE
            dbms_output.put_line('COMMENTS ERROR UPDATE - ' || systimestamp);
            UPDATE HBG_PROCESS_ORDERS_COMMENTS tkn
            SET STATUS = 'ERROR'
            WHERE EXISTS(SELECT hdr.source_transaction_id FROM HBG_PROCESS_ORDERS_HEADERS_FBDI hdr WHERE HDR.source_transaction_id = tkn.source_transaction_id and 
                            hdr.STATUS = 'ERROR' )
            AND BATCH_ID = p_SBI_UUID
            AND status = 'NEW'; 
            dbms_output.put_line('COMMENTS ERROR UPDATE - ' || systimestamp);
        ...
        ---------------------------------------------------------------------------- 
        --INSERT INTO HBG_PROCESS_ORDERS_DEBUG (TASK,TIME_STAMP) VALUES ('Derive and validate SAN ',systimestamp) ; 
            --COMMIT;
        IF p_debug = 'true' then
                DEBUG_MSG ( 
                    p_file_name => p_debug_filename,
       ...
       dbms_output.put_line('Derive and validate SAN - header null account shipto san - ' || systimestamp);  
        ...
        ---------------------------------------------------------------------------- 
        --INSERT INTO HBG_PROCESS_ORDERS_DEBUG (TASK,TIME_STAMP) VALUES ('Derive and validate account ',systimestamp) ; 
            --COMMIT;
         IF p_debug = 'true' then
                DEBUG_MSG ( 
                    p_file_name => p_debug_filename,
        ...
        /*=========================================================================*/
        --INSERT INTO HBG_PROCESS_ORDERS_DEBUG (TASK,TIME_STAMP) VALUES ('Derive and Validate Ship Site    ',systimestamp) ; 
            --COMMIT;
        IF p_debug = 'true' then
                DEBUG_MSG ( 
                    p_file_name => p_debug_filename,
        ...
        /*=========================================================================*/            
        --INSERT INTO HBG_PROCESS_ORDERS_DEBUG (TASK,TIME_STAMP) VALUES ('Derive and Validate Customer Info ',systimestamp) ; 
            --COMMIT;
        IF p_debug = 'true' then
                DEBUG_MSG ( 
                    p_file_name => p_debug_filename,
     ...
     /*=========================================================================*/   
    --INSERT INTO HBG_PROCESS_ORDERS_DEBUG (TASK,TIME_STAMP) VALUES ('Derive and Validate Bill to Info',systimestamp) ; 
            --COMMIT;
      IF p_debug = 'true' then
                DEBUG_MSG ( 
                    p_file_name => p_debug_filename,
    ...
    dbms_output.put_line('END Derive and Validate Bill to Info - ' || systimestamp); 
     ...
     /*=========================================================================*/   
        --INSERT INTO HBG_PROCESS_ORDERS_DEBUG (TASK,TIME_STAMP) VALUES ('Derive Address Text and Validate Ship Info  ',systimestamp) ; 
            --COMMIT;
       IF p_debug = 'true' then
                DEBUG_MSG ( 
                    p_file_name => p_debug_filename,
        ...
        dbms_output.put_line('null Address Text and Validate Ship Info - ' || systimestamp);
         ...
         ------------------------------------------------------------------------------------------------------
        /*                  UPDATE ONE TIME ADDRESS LINES WITH PRIMARY PARTY_SITE_ID                         */
        ------------------------------------------------------------------------------------------------------
         --INSERT INTO HBG_PROCESS_ORDERS_DEBUG (TASK,TIME_STAMP) VALUES ('UPDATE ONE TIME ADDRESS LINES WITH PRIMARY PARTY_SITE_ID  ',systimestamp) ; 
            --COMMIT;
        UPDATE HBG_PROCESS_ORDERS_ADDRESSES_FBDI adr
            SET PARTY_SITE_ID = (SELECT  hzps.party_site_id
                                    FROM HZ_PARTY_SITES hzps,
                                         HZ_CUST_ACCT_SITES_ALL hZcasa,
                                         HBG_PROCESS_ORDERS_ADDRESSES_FBDI adr2
                                    WHERE   hzcasa.CUST_ACCOUNT_ID = adr2.CUSTOMER_ID
                                        AND hzcasa.party_site_id     = hzps.party_site_id
                                        AND hzcasa.SHIP_TO_FLAG = 'P'
                                        AND adr2.SOURCE_TRANSACTION_LINE_ID = adr.SOURCE_TRANSACTION_LINE_ID
                                        AND adr2.SBI_UUID = adr.SBI_UUID
                                        AND adr2.ADDRESS_USE_TYPE = 'BILL_TO'
                                        )               
            WHERE   SBI_UUID = p_SBI_UUID
                and status = 'NEW'
                and adr.ADDRESS_USE_TYPE = 'SHIP_TO'
                and EXISTS(SELECT 1 FROM HBG_PROCESS_ORDERS_HDRS_EFF_FBDI hef 
                                                        WHERE CONTEXT_CODE = 'One Time Address' 
                                                        AND hef.sbi_uuid = adr.sbi_uuid 
                                                        and hef.SOURCE_TRANSACTION_ID = adr.SOURCE_TRANSACTION_ID);
                                                        
        ----------------------------------------------------------------------------------------
        /* UPDATE LINES WITH NO SHIP INFORMATION WITH ERROR STATUS                            */
        ----------------------------------------------------------------------------------------
        --INSERT INTO HBG_PROCESS_ORDERS_DEBUG (TASK,TIME_STAMP) VALUES ('UPDATE LINES WITH NO SHIP INFORMATION WITH ERROR STATUS  ',systimestamp) ; 
            --COMMIT;
        UPDATE HBG_PROCESS_ORDERS_ADDRESSES_FBDI adr
            SET STATUS = 'ERROR',
     ...
     /*=========================================================================*/   
        --INSERT INTO HBG_PROCESS_ORDERS_DEBUG (TASK,TIME_STAMP) VALUES ('INSERT ADDRESS HEADER BILL TO LINE ',systimestamp) ; 
            --COMMIT;
		...
		BEGIN
			...
			LOOP
				...
				BEGIN
					...
					/*-----------------------------------------------------------------------------
						MAPPING FOR FBDI ADDRESSES FILE - HEADER 
					-------------------------------------------------------------------------------*/
						...
						(
							...
							l_doo_addresses_hdr_interface_tb(i).SOURCE_ACCOUNT_NO,         			--SOURCE_ACCOUNT_NO,
                            l_doo_addresses_hdr_interface_tb(i).PARTY_ID,               			--PARTY_ID
							l_doo_addresses_hdr_interface_tb(i).CUSTOMER_ID,                        --CUSTOMER_ID,
                            l_doo_addresses_hdr_interface_tb(i).PARTY_SITE_ID,                      --PARTY_SITE_ID,
    ...
    /*=========================================================================*/	
        --INSERT INTO HBG_PROCESS_ORDERS_DEBUG (TASK,TIME_STAMP) VALUES ('SEND ADDRESS ERRORS TO ORDER TABLES',systimestamp) ; 
            --COMMIT;
        ...
        --UPDATE PAYMENT TOKEN TABLE
        dbms_output.put_line('PAYMENT TOKEN ERROR ADDRESS UPDATE - ' || systimestamp);
        UPDATE HBG_PROCESS_ORDERS_PAYMENT_TKN_FBDI tkn
        SET STATUS = 'ERROR'
        WHERE EXISTS(SELECT hdr.source_transaction_id FROM HBG_PROCESS_ORDERS_HEADERS_FBDI hdr WHERE HDR.source_transaction_id = tkn.source_transaction_id and 
                        hdr.STATUS = 'ERROR' )
        ...
        --UPDATE COMMENTS TABLE
        dbms_output.put_line('COMMENTS ERROR ADDRESS UPDATE - ' || systimestamp);
        UPDATE HBG_PROCESS_ORDERS_COMMENTS cmt
        SET STATUS = 'ERROR'
        WHERE EXISTS(SELECT hdr.source_transaction_id FROM HBG_PROCESS_ORDERS_HEADERS_FBDI hdr WHERE HDR.source_transaction_id = cmt.source_transaction_id and 
                        hdr.STATUS = 'ERROR' )
        AND BATCH_ID = p_SBI_UUID
        AND status = 'NEW'; 
  ...
  --------------------------------------------------------------------------------------------------------------------------------------------------------------------
        --INSERT INTO HBG_PROCESS_ORDERS_DEBUG (TASK,TIME_STAMP) VALUES ('LINE QUANTITY GRATER THAN 6 DIGITS',systimestamp) ; 
            --COMMIT;
        ...
        FOR l_rec in (
            ...
            EXCEPTION WHEN OTHERS THEN
                ...
                UPDATE HBG_PROCESS_ORDERS_PAYMENT_TKN_FBDI 
                    SET STATUS = 'ERROR',
                        ERROR_MSG = l_errbuf
                WHERE sbi_uuid = p_SBI_UUID
                    AND SOURCE_TRANSACTION_ID = l_rec.SOURCE_TRANSACTION_ID;
    ...
    /*=========================================================================*/
        --INSERT INTO HBG_PROCESS_ORDERS_DEBUG (TASK,TIME_STAMP) VALUES ('Update Status field with Validated',systimestamp) ; 
            --COMMIT;
        ...
        UPDATE HBG_PROCESS_ORDERS_COMMENTS 
        SET STATUS = 'VALIDATED'
        WHERE STATUS = 'NEW'
        AND BATCH_ID = p_SBI_UUID;
    ...
    EXCEPTION 
        WHEN le_custom_exception THEN
            --l_errbuf := SQLERRM;
            --INSERT INTO HBG_PROCESS_ORDERS_DEBUG (TASK,TIME_STAMP) VALUES ('le_custom_exception - ' || l_errbuf,systimestamp) ; 
            COMMIT;
			UPDATE HBG_STERLING_CONTROL 
				SET IMPORT_STATUS = 'ERROR',
					IMPORT_COMMENTS = l_errbuf
        ...
        WHEN OTHERS THEN
            retcode := 1;
            errbuf := sqlerrm;
            --INSERT INTO HBG_PROCESS_ORDERS_DEBUG (TASK,TIME_STAMP) VALUES ('le_custom_exception - ' || errbuf,systimestamp) ; 
            COMMIT;
            dbms_output.put_line(sqlerrm);
 ...
 /*-----------------------------------------------------------------------------------------------------
    ...
    DELETE FROM HBG_PROCESS_ORDERS_REPORT_STG WHERE BATCH_NAME = p_batch_name;
    COMMIT;
    ...
    UPDATE HBG_PROCESS_ORDERS_LINES_FBDI line SET STATUS = 'ERROR',
        ERROR_MSG = p_load_request_id
            WHERE SBI_UUID = p_batch_name
            AND EXISTS (SELECT rpt.SOURCE_TRANSACTION_LINE_ID FROM HBG_PROCESS_ORDERS_REPORT_STG rpt
                            WHERE rpt.SOURCE_TRANSACTION_LINE_ID = line.SOURCE_TRANSACTION_LINE_ID
                            and rpt.BATCH_NAME = p_batch_name
                            and line.SBI_UUID = rpt.BATCH_NAME
                            AND rpt.LOAD_REQUEST_ID =  p_load_request_id);
        COMMIT;
    ...
    PROCEDURE SUBMIT_ORDERS (p_source_transaction_id IN VARCHAR2,p_password IN VARCHAR2, p_erp_url VARCHAR2, errbuf out varchar2, retcode out number )
    AS
    ...
    l_source_transaction_sys VARCHAR2(50);
...
<ord:DraftOrder>
<ord:SourceTransactionIdentifier>'||p_source_transaction_id||'</ord:SourceTransactionIdentifier>
<ord:SourceTransactionSystem>'||l_source_transaction_sys||'</ord:SourceTransactionSystem>
</ord:DraftOrder>                   
</typ:submitDraftOrderRequest>
</typ:SubmitDraftOrder>
</soapenv:Body>
    ...
    /*PROCEDURE CALL_SUBMIT_JOB (
        p_source_transaction_id IN VARCHAR2,
    ...
    PROCEDURE CX_TO_STAGE_TABLES (
        ...
        CURSOR c_doo_header_all_interface IS
            SELECT 
                    ...
                    ACCOUNT_ID,
                    'CX'|| CX_ORDER_TYPE AS l_source_transaction_sys,
                    CURRENCY_CODE
      ...
      --DATA FOR SALES ORDER COMMENTS/ATTACHMENTS
        CURSOR c_doo_attachments_interface  IS
            SELECT
                SYSTEM_ORDER_ID,
                SHIPPING_INSTRUCTIONS AS COMMENT_TEXT,
                'CX'|| CX_ORDER_TYPE AS l_source_transaction_sys,
                'GEN' AS COMMENT_TYPE,
                1             AS COMMENT_NO
            FROM HBG_CX_ORDER_HEADER cmt
            WHERE 	INTEGRATION_STATUS_ERP = 'NEW'
                AND CREATION_DATE >= p_creation_date
                AND STATE = 'SUBMITTED'
                AND SHIPPING_INSTRUCTIONS IS NOT NULL
                
            UNION
            
            SELECT
                SYSTEM_ORDER_ID,
                'This product was sent to you by '|| ATTN AS COMMENT_TEXT,
                'CX'|| CX_ORDER_TYPE AS l_source_transaction_sys,
                'GEN' AS COMMENT_TYPE,
                2             AS COMMENT_NO
            FROM HBG_CX_ORDER_HEADER cmt
            ...
            SELECT 
                SYSTEM_ORDER_ID,
                ADDITIONAL_COMMENT AS COMMENT_TEXT,
                'CX'|| CX_ORDER_TYPE AS l_source_transaction_sys,
                'GEN' AS COMMENT_TYPE,
                3             AS COMMENT_NO
            FROM HBG_CX_ORDER_HEADER cmt
            WHERE 	INTEGRATION_STATUS_ERP = 'NEW'
                AND CREATION_DATE >= p_creation_date
                AND STATE = 'SUBMITTED'
                AND ADDITIONAL_COMMENT IS NOT NULL;
        ...
        --DATA FOR SALES ORDER ADDRESSES
        CURSOR c_doo_addresses_all_interface IS
            SELECT 
                SOURCE_TRANSACTION_ID,
                SOURCE_TRANSACTION_SYSTEM,
                'SHIP_TO' AS ADDRESS_USE_TYPE,
                SBI_UUID,
                STATUS,
                SOURCE_ACCOUNT_NO,
                SOURCE_SYSTEM
            FROM HBG_PROCESS_ORDERS_HEADERS_FBDI
            WHERE SBI_UUID = 'CX' || p_instanceid
              AND SOURCE_SYSTEM = 'CX' 
              AND STATUS = 'NEW'
   ...
   /*-----------------------------------------------------------------------------------------------------
    ...
    ------------------------------------------------------------------------------------------------------*/
		...
		l_fbdi_errbuf		                    VARCHAR2(500);
        l_ftp_filename                          VARCHAR2(70);
        p_debug_filename	                    VARCHAR2(100) := 'ProcessOrders_' || to_char(sysdate, 'YYMMDDHH24MMSS');
    /*-----------------------------------------------------------------------------------------------------
            EXCEPTIONS
    ------------------------------------------------------------------------------------------------------*/
        le_custom_exception EXCEPTION;
                    ...
                    BEGIN
                        FORALL i IN 1 .. l_doo_header_all_interface_tb.COUNT SAVE EXCEPTIONS
                            ...
                            (
                                ...
                                l_doo_header_all_interface_tb(i).l_source_transaction_sys,      --SOURCE_TRANSACTION_SYSTEM,
                                ...
                                'HBG US BU',                                                    --REQUESTING_BUSINESS_UNIT,
                                'HBG_REGULAR',                                                  --TRANSACTION_TYPE_CODE
                    ...
                    BEGIN
                        FORALL i IN 1 .. l_doo_hdr_effs_all_interface_tb.COUNT SAVE EXCEPTIONS
                            ...
                            (
                                ...
                                l_doo_hdr_effs_all_interface_tb(i).l_source_transaction_sys,        --SOURCE_TRANSACTION_SYSTEM, 
                                'General',                          								--CONTEXT_CODE,             
                    ...
                    BEGIN
                        FORALL i IN 1 .. l_doo_hdr_effs_all_interface_tb.COUNT SAVE EXCEPTIONS
                            ...
                            (
                                ...
                                l_doo_hdr_effs_all_interface_tb(i).l_source_transaction_sys,                --SOURCE_TRANSACTION_SYSTEM, 
                                'EDI General',                          							        --CONTEXT_CODE,             
                                TO_DATE(l_doo_hdr_effs_all_interface_tb(i).CANCELLATION_DATE,'YYYY/MM/DD'), --ATTRIBUTE_TIMESTAMP1
                                'CX',															            --SOURCE_SYSTEM   
                                'NEW',     															        --STATUS						
                                'CX' || p_instanceid, 												        --SBI_UUID   
                                sysdate,                                                                    --CREATION_DATE, 
				...
				BEGIN
					FORALL i IN 1 .. l_doo_hdr_effs_all_interface_tb.COUNT SAVE EXCEPTIONS
						...
						(
                            ...
                            l_doo_hdr_effs_all_interface_tb(i).l_source_transaction_sys,              	--SOURCE_TRANSACTION_SYSTEM,
							'EDI Address',                          									--CONTEXT_CODE,            
							l_doo_hdr_effs_all_interface_tb(i).ATTN,								    --ATTRIBUTE_CHAR1,         
							l_doo_hdr_effs_all_interface_tb(i).SHIPPING_ADDRESS1,						--ATTRIBUTE_CHAR2,          
							l_doo_hdr_effs_all_interface_tb(i).SHIPPING_ADDRESS2,    					--ATTRIBUTE_CHAR3,          
							l_doo_hdr_effs_all_interface_tb(i).SHIPPING_ADDRESS3,						--ATTRIBUTE_CHAR4,          
							l_doo_hdr_effs_all_interface_tb(i).SHIPPING_ADDRESS4,						--ATTRIBUTE_CHAR5,          
							l_doo_hdr_effs_all_interface_tb(i).SHIPPING_CITY,							--ATTRIBUTE_CHAR6,          
							l_doo_hdr_effs_all_interface_tb(i).SHIPPING_STATE,							--ATTRIBUTE_CHAR7,         
							l_doo_hdr_effs_all_interface_tb(i).SHIPPING_POSTAL_CODE,					--ATTRIBUTE_CHAR8,          
							l_doo_hdr_effs_all_interface_tb(i).SHIPPING_COUNTRY,   						--ATTRIBUTE_CHAR9,          
							l_doo_hdr_effs_all_interface_tb(i).DEST_ATTN,   						    --ATTRIBUTE_CHAR10, 
                            l_doo_hdr_effs_all_interface_tb(i).SHIP_TO_NOTIFICATION_EMAIL,	            --ATTRIBUTE_CHAR11,
							'CX',                             									        --SOURCE_SYSTEM,            
							'NEW',                                  									--STATUS,									
							'CX' || p_instanceid,                    									--SBI_UUID    
                            sysdate,                                                                    --CREATION_DATE, 
                    ...
                    BEGIN
                        ...
                        -------------------------------------------------------------------------------*/
                            ...
                            (
                                ...
                                l_doo_hdr_effs_all_interface_tb(i).SHIP_TO_NOTIFICATION_EMAIL,	        --ATTRIBUTE_CHAR13,
                                'CX',                                                                   --SOURCE_SYSTEM,            
                                'NEW',                                                                  --STATUS,										
                                'CX' || p_instanceid,                                                    --SBI_UUID 
                                sysdate,                                                            --CREATION_DATE, 
        ...
        ------------------------------------------------------------------------------------------------------*/
            ...
            BEGIN
                    ...
                    BEGIN
                        FORALL i IN 1 .. l_doo_lines_all_interface_tb.COUNT SAVE EXCEPTIONS
                            ...
                            (
                                ...
                                l_doo_lines_all_interface_tb(i).l_source_transaction_sys,                   --SOURCE_TRANSACTION_SYSTEM,
                                ...
                                'HBG US BU',                                                   				--BUSINESS_UNIT_NAME,			
                                'ORDER',                                                       				--TRANSACTION_CATEGORY_CODE,
                                l_doo_lines_all_interface_tb(i).UNIT_PRICE,                                 --UNIT_LIST_PRICE
                                l_doo_lines_all_interface_tb(i).UNIT_PRICE,                                 --UNIT_SELLING_PRICE
                                l_doo_lines_all_interface_tb(i).PRICE,                                      --EXTENDED_AMOUNT
                                'CX' || p_instanceid ,                                         				--SBI_UUID,					
                                'NEW',                                                         				--STATUS,						
                                'IMMEDIATE',                                                   				--PAYMENT_TERM,            
                                'CX',                                                          				--SOURCE_SYSTEM,            
                                l_doo_lines_all_interface_tb(i).ACCOUNT_ID,                    				--SOURCE_ACCOUNT_NO,        
                                to_char(nvl(l_doo_lines_all_interface_tb(i).ARRIVE_BY,sysdate+1),'YYYY/MM/DD HH24:MI:SS'), --REQUESTED_SHIP_DATE,      
                                to_char(nvl(l_doo_lines_all_interface_tb(i).ARRIVE_BY,sysdate+1),'YYYY/MM/DD HH24:MI:SS'),  --SCHEDULE_SHIP_DATE,
                                sysdate,                                                            --CREATION_DATE, 
                                sysdate,                                                            --LAST_UPDATED_DATE,
                                'HBG_INTEGRATION',                                                  --CREATED_BY,
                                'HBG_INTEGRATION'                                                   --LAST_UPDATED_BY,
                ...
                -------------------------------------------------------------------------------*/
                 ...
                 /*-----------------------------------------------------------------------------
                    MAPPING FOR CHARGE FBDI FILE 
                -------------------------------------------------------------------------------*/
                    BEGIN
                        FORALL i IN 1 .. l_doo_lines_all_interface_tb.COUNT SAVE EXCEPTIONS
                            INSERT INTO HBG_PROCESS_ORDERS_CHARGES_FBDI
                            (
                                SOURCE_TRANSACTION_ID, 	
                                SOURCE_TRANSACTION_SYSTEM,
                                SOURCE_TRANSACTION_LINE_ID,		
                            ...
                            (
                                ...
                                l_doo_lines_all_interface_tb(i).LINE_ID,                                    --SOURCE_TRANSACTION_LINE_ID,
                                l_doo_lines_all_interface_tb(i).LINE_ID,									--SOURCE_TRANSACTION_SCHEDULE_ID,
                                l_doo_lines_all_interface_tb(i).LINE_ID,								    --SOURCE_TRANSACTION_CHARGE_ID
                                'QP_SALE_PRICE',								                            --CHARGE_DEFINITION_CODE
                                'ORA_SALE',                            	                                    --CHARGE_TYPE_CODE,		
                                'ORA_PRICE',                                                                --CHARGE_SUBTYPE_CODE
                                1,                                                                          --SEQUENCE_NUMBER
                                'ONE_TIME',                                                                 --PRICE_TYPE_CODE
                                l_doo_lines_all_interface_tb(i).QUANTITY,                      				--PRICED_QUANTITY,			
                                'Ea',                                                          				--PRICED_QUANTITY_UOM_CODE,
                                'Y',                                                                        --PRIMARY_FLAG
                                'PRICE',                                                                    --APPLY_TO
                                'N',                                                                        --ROLLUP_FLAG
                                l_doo_lines_all_interface_tb(i).CURRENCY_CODE,                              --CHARGE_CURRENCY_CODE
                                'CX' || p_instanceid ,                                         				--SBI_UUID,					
                                'NEW',                                                         				--STATUS,						         
                                'CX',                                                          				--SOURCE_SYSTEM,  
                                sysdate,                                                            --CREATION_DATE, 
                        ...
                        IF p_debug = 'true' then
                            DEBUG_MSG ( 
                                p_file_name => p_debug_filename,
                                p_debug_msg => 'BEGIN PROCESS CHARGES DATA - Sucessfully inserted count '||l_count_success );
                        end if;
                    ...
                    EXCEPTION 
                        ...
                        -------------------------------------------------------------------------------*/
                            FOR i IN 1 .. SQL%BULK_EXCEPTIONS.COUNT LOOP
                            --dbms_output.put_line('error line');
                            l_errbuf := 'ERROR INSERTING CHARGES - ' || SQLERRM(-SQL%BULK_EXCEPTIONS(i).ERROR_CODE);
                            l_update_errmsg := UPDATE_ERROR_MSG	(	l_errbuf, 
                                                                    l_doo_lines_all_interface_tb(i).ORDER_HEADER_SOURCE_ID, 
                                                                    l_doo_lines_all_interface_tb(i).ORDER_HEADER_SOURCE_ID, 
                                                                    'CX' || p_instanceid, 
                            ...
                            IF p_debug = 'true' then
                                DEBUG_MSG ( 
                                    p_file_name => p_debug_filename,
                                    p_debug_msg => 'BEGIN PROCESS CHARGE DATA - '||l_count_fail||' orders failed');
                            end if;
    
                            IF p_debug = 'true' then
                                DEBUG_MSG ( 
                                    p_file_name => p_debug_filename,
                                    p_debug_msg => 'BEGIN PROCESS CHARGE DATA - '||l_count_success||' orders succeeded');
                            end if;
                ...
                /*-----------------------------------------------------------------------------
                    MAPPING FOR CHARGE COMPS LIST PRICE FBDI FILE 
                -------------------------------------------------------------------------------*/
                    BEGIN
                        FORALL i IN 1 .. l_doo_lines_all_interface_tb.COUNT SAVE EXCEPTIONS
                            INSERT INTO HBG_PROCESS_ORDERS_CHARGES_COMPS_FBDI
                            (
                                SOURCE_TRANSACTION_ID, 	
                                SOURCE_TRANSACTION_SYSTEM,
                                SOURCE_TRANSACTION_LINE_ID,		
                            ...
                            (
                                ...
                                l_doo_lines_all_interface_tb(i).LINE_ID,									--SOURCE_TRANSACTION_SCHEDULE_ID,
                                l_doo_lines_all_interface_tb(i).LINE_ID,								    --SOURCE_TRANSACTION_CHARGE_ID
                                'LP'||l_doo_lines_all_interface_tb(i).LINE_ID,								--SOURCE_CHARGE_COMPONENT_ID
                                1,                                                                          --SEQUENCE_NUMBER
                                l_doo_lines_all_interface_tb(i).CURRENCY_CODE,                              --CHARGE_CURRENCY_CODE
                                l_doo_lines_all_interface_tb(i).PRICE,                                      --CHARGE_CURRENCY_EXT_AMOUNT 
                                l_doo_lines_all_interface_tb(i).UNIT_PRICE,                                 --CHARGE_CURRENCY_UNIT_PRICE 
                                l_doo_lines_all_interface_tb(i).CURRENCY_CODE,                              --HEADER_CURRENCY_CODE
                                l_doo_lines_all_interface_tb(i).PRICE,                                      --HEADER_CURRENCY_EXT_AMOUNT 
                                l_doo_lines_all_interface_tb(i).UNIT_PRICE,                                 --HEADER_CURRENCY_UNIT_PRICE 
                                'QP_LIST_PRICE',                            	                            --PRICE_ELEMENT_CODE,		
                                'LIST_PRICE',                                                               --PRICE_ELEMENT_USAGE_CODE
                                'CX' || p_instanceid ,                                         				--SBI_UUID,					
                                'NEW',                                                         				--STATUS,						         
                                'CX',                                                          				--SOURCE_SYSTEM,
                                sysdate,                                                            --CREATION_DATE, 
                        ...
                        IF p_debug = 'true' then
                            DEBUG_MSG ( 
                                p_file_name => p_debug_filename,
                                p_debug_msg => 'BEGIN PROCESS CHARGES COMPS LIST_PRICE DATA - Sucessfully inserted count '||l_count_success );
                        end if;
                    ...
                    EXCEPTION 
                        ...
                        -------------------------------------------------------------------------------*/
                            FOR i IN 1 .. SQL%BULK_EXCEPTIONS.COUNT LOOP
                            --dbms_output.put_line('error line');
                            l_errbuf := 'ERROR INSERTING CHARGES COMPS LIST_PRICE - ' || SQLERRM(-SQL%BULK_EXCEPTIONS(i).ERROR_CODE);
                            l_update_errmsg := UPDATE_ERROR_MSG	(	l_errbuf, 
                                                                    l_doo_lines_all_interface_tb(i).ORDER_HEADER_SOURCE_ID, 
                                                                    l_doo_lines_all_interface_tb(i).ORDER_HEADER_SOURCE_ID, 
                                                                    'CX' || p_instanceid, 
                            ...
                            IF p_debug = 'true' then
                                DEBUG_MSG ( 
                                    p_file_name => p_debug_filename,
                                    p_debug_msg => 'BEGIN PROCESS CHARGE COMPS LIST PRICE DATA - '||l_count_fail||' orders failed');
                            end if;
    
                            IF p_debug = 'true' then
                                DEBUG_MSG ( 
                                    p_file_name => p_debug_filename,
                                    p_debug_msg => 'BEGIN PROCESS CHARGE COMPS LIST PRICE DATA - '||l_count_success||' orders succeeded');
                            end if;
                ...
                /*-----------------------------------------------------------------------------
                    MAPPING FOR CHARGE COMPS NET PRICE FBDI FILE 
                -------------------------------------------------------------------------------*/
                    BEGIN
                        FORALL i IN 1 .. l_doo_lines_all_interface_tb.COUNT SAVE EXCEPTIONS
                            INSERT INTO HBG_PROCESS_ORDERS_CHARGES_COMPS_FBDI
                            ...
                            (
                                ...
                                l_doo_lines_all_interface_tb(i).LINE_ID,									        --SOURCE_TRANSACTION_SCHEDULE_ID,
                                l_doo_lines_all_interface_tb(i).LINE_ID,								            --SOURCE_TRANSACTION_CHARGE_ID
                                'NP'||l_doo_lines_all_interface_tb(i).LINE_ID,								        --SOURCE_CHARGE_COMPONENT_ID
                                2,                                                                                  --SEQUENCE_NUMBER
                                l_doo_lines_all_interface_tb(i).CURRENCY_CODE,                                      --CHARGE_CURRENCY_CODE
                                l_doo_lines_all_interface_tb(i).PRICE, --+ l_doo_lines_all_interface_tb(i).TAX_AMOUNT, --CHARGE_CURRENCY_EXT_AMOUNT
                                l_doo_lines_all_interface_tb(i).UNIT_PRICE,                                         --CHARGE_CURRENCY_UNIT_PRICE
                                l_doo_lines_all_interface_tb(i).CURRENCY_CODE,                                      --HEADER_CURRENCY_CODE
                                l_doo_lines_all_interface_tb(i).PRICE,-- + l_doo_lines_all_interface_tb(i).TAX_AMOUNT,--HEADER_CURRENCY_EXT_AMOUNT 
                                l_doo_lines_all_interface_tb(i).UNIT_PRICE,                                         --HEADER_CURRENCY_UNIT_PRICE
                                'QP_NET_PRICE',--'QP_NET_PRICE_PLUS_TAX',                            	                            --PRICE_ELEMENT_CODE,		
                                'NET_PRICE',--'NET_PRICE_PLUS_TAX',                                                               --PRICE_ELEMENT_USAGE_CODE
                                'CX' || p_instanceid ,                                         				        --SBI_UUID,					
                                'NEW',                                                         				        --STATUS,						         
                                'CX',                                                          				        --SOURCE_SYSTEM,
                                sysdate,                                                            --CREATION_DATE, 
                        ...
                        IF p_debug = 'true' then
                            DEBUG_MSG ( 
                                p_file_name => p_debug_filename,
                                p_debug_msg => 'BEGIN PROCESS CHARGES COMPS NET_PRICE DATA - Sucessfully inserted count '||l_count_success );
                        end if;
                    ...
                    EXCEPTION 
                        ...
                        -------------------------------------------------------------------------------*/
                            FOR i IN 1 .. SQL%BULK_EXCEPTIONS.COUNT LOOP
                            --dbms_output.put_line('error line');
                            l_errbuf := 'ERROR INSERTING CHARGES COMPS NET_PRICE - ' || SQLERRM(-SQL%BULK_EXCEPTIONS(i).ERROR_CODE);
                            l_update_errmsg := UPDATE_ERROR_MSG	(	l_errbuf, 
                                                                    l_doo_lines_all_interface_tb(i).ORDER_HEADER_SOURCE_ID, 
                                                                    l_doo_lines_all_interface_tb(i).ORDER_HEADER_SOURCE_ID, 
                                                                    'CX' || p_instanceid, 
                            ...
                            IF p_debug = 'true' then
                                DEBUG_MSG ( 
                                    p_file_name => p_debug_filename,
                                    p_debug_msg => 'BEGIN PROCESS CHARGE COMPS NET PRICE DATA - '||l_count_fail||' orders failed');
                            end if;
    
                            IF p_debug = 'true' then
                                DEBUG_MSG ( 
                                    p_file_name => p_debug_filename,
                                    p_debug_msg => 'BEGIN PROCESS CHARGE COMPS NET PRICE DATA - '||l_count_success||' orders succeeded');
                            end if;
                ...
                /*-----------------------------------------------------------------------------
                    MAPPING FOR CHARGE COMPS TAX FBDI FILE 
                -------------------------------------------------------------------------------*/
                    BEGIN
                        FORALL i IN 1 .. l_doo_lines_all_interface_tb.COUNT SAVE EXCEPTIONS
                            INSERT INTO HBG_PROCESS_ORDERS_CHARGES_COMPS_FBDI
                            ...
                            (
                                ...
                                l_doo_lines_all_interface_tb(i).LINE_ID,									        --SOURCE_TRANSACTION_SCHEDULE_ID,
                                l_doo_lines_all_interface_tb(i).LINE_ID,								            --SOURCE_TRANSACTION_CHARGE_ID
                                'TX'||l_doo_lines_all_interface_tb(i).LINE_ID,								        --SOURCE_CHARGE_COMPONENT_ID
                                3,                                                                                  --SEQUENCE_NUMBER
                                l_doo_lines_all_interface_tb(i).CURRENCY_CODE,                                      --CHARGE_CURRENCY_CODE
                                l_doo_lines_all_interface_tb(i).TAX_AMOUNT,                                         --CHARGE_CURRENCY_EXT_AMOUNT 
                                l_doo_lines_all_interface_tb(i).TAX_AMOUNT/l_doo_lines_all_interface_tb(i).QUANTITY,--CHARGE_CURRENCY_UNIT_PRICE 
                                l_doo_lines_all_interface_tb(i).CURRENCY_CODE,                                      --HEADER_CURRENCY_CODE
                                l_doo_lines_all_interface_tb(i).TAX_AMOUNT,                                         --HEADER_CURRENCY_EXT_AMOUNT 
                                l_doo_lines_all_interface_tb(i).TAX_AMOUNT/l_doo_lines_all_interface_tb(i).QUANTITY,--HEADER_CURRENCY_UNIT_PRICE
                                'QP_EXCLUSIVE_TAX',                            	                                    --PRICE_ELEMENT_CODE,		
                                'EXCLUSIVE_TAX',                                                                    --PRICE_ELEMENT_USAGE_CODE
                                'CX' || p_instanceid ,                                         				        --SBI_UUID,					
                                'NEW',                                                         				        --STATUS,						         
                                'CX',                                                          				        --SOURCE_SYSTEM,  
                                sysdate,                                                            --CREATION_DATE, 
                        ...
                        IF p_debug = 'true' then
                            DEBUG_MSG ( 
                                p_file_name => p_debug_filename,
                                p_debug_msg => 'BEGIN PROCESS CHARGES COMPS TAX DATA - Sucessfully inserted count '||l_count_success );
                        end if;
                    ...
                    EXCEPTION 
                        ...
                        -------------------------------------------------------------------------------*/
                            FOR i IN 1 .. SQL%BULK_EXCEPTIONS.COUNT LOOP
                            --dbms_output.put_line('error line');
                            l_errbuf := 'ERROR INSERTING CHARGES COMPS TAX - ' || SQLERRM(-SQL%BULK_EXCEPTIONS(i).ERROR_CODE);
                            l_update_errmsg := UPDATE_ERROR_MSG	(	l_errbuf, 
                                                                    l_doo_lines_all_interface_tb(i).ORDER_HEADER_SOURCE_ID, 
                                                                    l_doo_lines_all_interface_tb(i).ORDER_HEADER_SOURCE_ID, 
                                                                    'CX' || p_instanceid, 
                            ...
                            IF p_debug = 'true' then
                                DEBUG_MSG ( 
                                    p_file_name => p_debug_filename,
                                    p_debug_msg => 'BEGIN PROCESS CHARGE COMPS TAX DATA - '||l_count_fail||' orders failed');
                            end if;
    
                            IF p_debug = 'true' then
                                DEBUG_MSG ( 
                                    p_file_name => p_debug_filename,
                                    p_debug_msg => 'BEGIN PROCESS CHARGE COMPS TAX DATA - '||l_count_success||' orders succeeded');
                            end if;
                ...
                /*-----------------------------------------------------------------------------
                    MAPPING FOR CHARGE COMPS NET PRICE PLUS TAX FBDI FILE 
                -------------------------------------------------------------------------------*/
                    BEGIN
                        FORALL i IN 1 .. l_doo_lines_all_interface_tb.COUNT SAVE EXCEPTIONS
                            INSERT INTO HBG_PROCESS_ORDERS_CHARGES_COMPS_FBDI
                            ...
                            (
                                ...
                                l_doo_lines_all_interface_tb(i).LINE_ID,									        --SOURCE_TRANSACTION_SCHEDULE_ID,
                                l_doo_lines_all_interface_tb(i).LINE_ID,								            --SOURCE_TRANSACTION_CHARGE_ID
                                'NT'||l_doo_lines_all_interface_tb(i).LINE_ID,								        --SOURCE_CHARGE_COMPONENT_ID
                                4,                                                                                  --SEQUENCE_NUMBER
                                l_doo_lines_all_interface_tb(i).CURRENCY_CODE,                                      --CHARGE_CURRENCY_CODE
                                l_doo_lines_all_interface_tb(i).PRICE + l_doo_lines_all_interface_tb(i).TAX_AMOUNT, --CHARGE_CURRENCY_EXT_AMOUNT
                                l_doo_lines_all_interface_tb(i).UNIT_PRICE + 
                                l_doo_lines_all_interface_tb(i).TAX_AMOUNT/l_doo_lines_all_interface_tb(i).QUANTITY,--CHARGE_CURRENCY_UNIT_PRICE
                                l_doo_lines_all_interface_tb(i).CURRENCY_CODE,                                      --HEADER_CURRENCY_CODE
                                l_doo_lines_all_interface_tb(i).PRICE + l_doo_lines_all_interface_tb(i).TAX_AMOUNT, --HEADER_CURRENCY_EXT_AMOUNT 
                                l_doo_lines_all_interface_tb(i).UNIT_PRICE + 
                                l_doo_lines_all_interface_tb(i).TAX_AMOUNT/l_doo_lines_all_interface_tb(i).QUANTITY,--HEADER_CURRENCY_UNIT_PRICE
                                'QP_NET_PRICE_PLUS_TAX',                            	                            --PRICE_ELEMENT_CODE,		
                                'NET_PRICE_PLUS_TAX',                                                               --PRICE_ELEMENT_USAGE_CODE
                                'CX' || p_instanceid ,                                         				        --SBI_UUID,					
                                'NEW',                                                         				        --STATUS,						         
                                'CX',                                                          				        --SOURCE_SYSTEM,
                                sysdate,                                                            --CREATION_DATE, 
        ...
        ------------------------------------------------------------------------------------------------------*/
            ...
            dbms_output.put_line(systimestamp); 
                IF p_debug = 'true' then
                    DEBUG_MSG ( 
                        p_file_name => p_debug_filename,
                        p_debug_msg => 'BEGIN PROCESS ADDRESSES DATA - OPEN c_doo_addresses_all_interface' );
                end if;
                dbms_output.put_line('header');
                OPEN c_doo_addresses_all_interface;
                LOOP
                    ...
                    IF p_debug = 'true' then
                        DEBUG_MSG ( 
                            p_file_name => p_debug_filename,
                            p_debug_msg => 'BEGIN PROCESS ADDRESSES DATA - Bulk Collect count '||l_count );
                    end if;
                    ...
                    BEGIN
                        FORALL i IN 1 .. l_doo_addresses_all_interface_tb.COUNT SAVE EXCEPTIONS
                            ...
                            (
                                l_doo_addresses_all_interface_tb(i).SOURCE_TRANSACTION_ID,	    --SOURCE_TRANSACTION_ID, 	
                                l_doo_addresses_all_interface_tb(i).SOURCE_TRANSACTION_SYSTEM,  --SOURCE_TRANSACTION_SYSTEM,
                                l_doo_addresses_all_interface_tb(i).ADDRESS_USE_TYPE,           --ADDRESS_USE_TYPE,
                                l_doo_addresses_all_interface_tb(i).SBI_UUID,                   --SBI_UUID,
                                l_doo_addresses_all_interface_tb(i).STATUS,                     --STATUS,			         
                                l_doo_addresses_all_interface_tb(i).SOURCE_ACCOUNT_NO,          --SOURCE_ACCOUNT_NO,                                                                    --SUBMIT_FLAG,                
                                l_doo_addresses_all_interface_tb(i).SOURCE_SYSTEM,               --SOURCE_SYSTEM
                                sysdate,                                                            --CREATION_DATE, 
                                sysdate,                                                            --LAST_UPDATED_DATE,
                                'HBG_INTEGRATION',                                                  --CREATED_BY,
                                'HBG_INTEGRATION'                                                   --LAST_UPDATED_BY,
                    ...
                    EXCEPTION 
                        ...
                        -------------------------------------------------------------------------------*/
                            FOR i IN 1 .. SQL%BULK_EXCEPTIONS.COUNT LOOP
                            l_errbuf := 'ERROR INSERTING ADDRESSES - ' || SQLERRM(-SQL%BULK_EXCEPTIONS(i).ERROR_CODE);
                            l_update_errmsg := UPDATE_ERROR_MSG	(	l_errbuf, 
                                                                    l_doo_addresses_all_interface_tb(i).SOURCE_TRANSACTION_ID, 
                                                                    l_doo_addresses_all_interface_tb(i).SOURCE_TRANSACTION_ID, 
                                                                    'CX' || p_instanceid, 
                ...
                IF p_debug = 'true' then
                    DEBUG_MSG ( 
                        p_file_name => p_debug_filename,
                        p_debug_msg => 'BEGIN PROCESS ADDRESSES DATA - '||l_count_fail||' orders failed');
                end if;
    
                IF p_debug = 'true' then
                    DEBUG_MSG ( 
                        p_file_name => p_debug_filename,
                        p_debug_msg => 'BEGIN PROCESS ADDRESSES DATA - '||l_count_success||' orders succeeded');
                end if;
                CLOSE c_doo_addresses_all_interface;
            ...
            -------------------------------------------------------------------------------------
                BEGIN
					FORALL i IN 1 .. l_doo_attachments_interface_tb.count SAVE EXCEPTIONS
						...
						VALUES
						(
							l_doo_attachments_interface_tb(i).SYSTEM_ORDER_ID,	                --SOURCE_TRANSACTION_ID, 	
							l_doo_attachments_interface_tb(i).l_source_transaction_sys,         --SOURCE_TRANSACTION_SYSTEM,
							'TEXT',                                                             --DATA_TYPE_CODE,
							l_doo_attachments_interface_tb(i).COMMENT_TYPE,                     --CATEGORY_NAME,
							'text/plain',                                                       --CONTENT_TYPE,			
							l_doo_attachments_interface_tb(i).COMMENT_TEXT,                     --UPLOADED_TEXT,
                            l_doo_attachments_interface_tb(i).COMMENT_NO,                       --COMMENT_NO
							'CX' || p_instanceid,                                               --BATCH_ID,                   
							'NEW',                                                              --STATUS,					         
							'STERLING',                                                         --SOURCE_SYSTEM,              
							sysdate,                                                            --CREATION_DATE, 
                            sysdate,                                                            --LAST_UPDATED_DATE,
				...
				EXCEPTION 
					...
					-------------------------------------------------------------------------------*/
						...
						l_errbuf := 'ERROR INSERTING COMMETS - ' || SQLERRM(-SQL%BULK_EXCEPTIONS(i).ERROR_CODE);
						l_update_errmsg := UPDATE_ERROR_MSG	(	l_errbuf, 
																l_doo_attachments_interface_tb(i).SYSTEM_ORDER_ID, 
																l_doo_attachments_interface_tb(i).SYSTEM_ORDER_ID, 
																'CX' || p_instanceid, 
																'NEW',
                                                                'CX');
		...
		END;
         ...
         generate_fbdi (
                    p_SBI_UUID 	      => 'CX' || p_instanceid,
                    p_source_system   => 'CX',
                    errbuf            => l_fbdi_errbuf,
                    retcode           => l_fbdi_retcode,
                    ftp_filename      => l_ftp_filename 
            
                );
           
           IF l_fbdi_retcode = 1 THEN 
                l_errbuf := l_fbdi_errbuf;
                RAISE le_custom_exception;
            END IF;
            
            UPDATE HBG_CX_ORDER_HEADER 
                SET INTEGRATION_STATUS_ERP = 'SENT'
            WHERE INTEGRATION_STATUS_ERP = 'NEW'
              AND SYSTEM_ORDER_ID IN (SELECT SOURCE_TRANSACTION_ID 
                                        FROM HBG_PROCESS_ORDERS_HEADERS_FBDI 
                                        WHERE SBI_UUID = 'CX' || p_instanceid );
            ...
            UPDATE HBG_CX_ORDER_LINE 
                SET INTEGRATION_STATUS_ERP = 'SENT'
            WHERE INTEGRATION_STATUS_ERP = 'NEW'
              AND LINE_ID IN (SELECT SOURCE_TRANSACTION_LINE_ID 
                                        FROM HBG_PROCESS_ORDERS_LINES_FBDI 
                                        WHERE SBI_UUID = 'CX' || p_instanceid );
            COMMIT;
            
            UPDATE HBG_CX_ORDERS_JOB_TRACKER SET STATUS = 'COMPLETE' WHERE instanceid = p_instanceid;
            COMMIT;
    ...
    PROCEDURE SEND_ORDER_ATTACHMENTS (p_batch_id IN VARCHAR2, p_password IN VARCHAR2, p_erp_url VARCHAR2 )
    AS
    ...
    l_request_clob  CLOB;
    l_url           VARCHAR2(1000);
    --erp_url    https://fa-ehfx-dev3-saasfaprod1.fa.ocs.oraclecloud.com:443
    ...
    BEGIN
        ...
        FOR l_rec in (
            SELECT
                ...
                ,CONTENT_TYPE					 
                ,UPLOADED_TEXT					 
            FROM HBG_PROCESS_ORDERS_COMMENTS
            WHERE BATCH_ID = p_batch_id
            AND STATUS = 'VALIDATED')
        LOOP
            BEGIN
            
                l_url := p_erp_url || '/fscmRestApi/resources/11.13.18.05/salesOrdersForOrderHub/'
                                   || l_rec.SOURCE_TRANSACTION_SYSTEM || ':' 
                                   || l_rec.SOURCE_TRANSACTION_ID
                                   ||'/child/attachments';
                    
                l_request_clob := to_clob(      '{"DatatypeCode": "'            ||l_rec.DATA_TYPE_CODE 
                                            || '","CategoryName": "'            ||l_rec.CATEGORY_NAME
                                            || '","UploadedFileContentType": "' ||l_rec.CONTENT_TYPE
                                            || '","UploadedText": "'            ||l_rec.UPLOADED_TEXT ||'"}');
                ...
                dbms_output.put_line('MAKE REQUEST - ' || l_url); 
                ...
                UPDATE HBG_PROCESS_ORDERS_COMMENTS SET STATUS = 'SUBMITED'
                WHERE   BATCH_ID = p_batch_id 
                    AND SOURCE_TRANSACTION_ID = l_rec.SOURCE_TRANSACTION_ID 
                    AND COMMENT_NO = l_rec.COMMENT_NO;
                
            EXCEPTION WHEN OTHERS THEN
                dbms_output.put_line('SQLERRM - ' || sqlerrm);
                l_sqlerrm := utl_http.get_detailed_sqlerrm;
                dbms_output.put_line('SQLERRM - ' || l_sqlerrm);
                UPDATE HBG_PROCESS_ORDERS_COMMENTS 
                    SET STATUS = 'ERROR',
                        ERROR_MSG = 'Error: ' || l_sqlerrm
                    WHERE   BATCH_ID = p_batch_id 
                        AND SOURCE_TRANSACTION_ID = l_rec.SOURCE_TRANSACTION_ID 
                        AND COMMENT_NO = l_rec.COMMENT_NO;
                COMMIT;
    ...
    WHERE EXISTS ( SELECT 1 
                   FROM HBG_PROCESS_ORDERS_COMMENTS cmt 
                   WHERE STATUS = 'ERROR' 
                     AND hdr.SOURCE_TRANSACTION_ID = cmt.SOURCE_TRANSACTION_ID 
                     AND hdr.SBI_UUID = cmt.BATCH_ID )
        AND SBI_UUID = p_batch_id;
        
    UPDATE HBG_PROCESS_ORDERS_ATTACHMENT_JOB_TRACKER
       SET STATUS = 'COMPLETE'
       WHERE BATCH_ID = p_batch_id;
       COMMIT;
    ...
    END SEND_ORDER_ATTACHMENTS;
        ...
        l_errbuf  VARCHAR2(3000);
        l_retcode VARCHAR2(1);
        l_tracker HBG_PROCESS_ORDERS_ATTACHMENT_JOB_TRACKER%rowtype;
        l_timestamp VARCHAR2(20);
    BEGIN
        errbuf := NULL;
        retcode := 0;
        ...
        job_name := 'HBG_PROCESS_ORDERS_' || l_timestamp;
        dbms_scheduler.create_job(job_name => 'HBG_PROCESS_ORDERS_' || l_timestamp, job_type => 'PLSQL_BLOCK', job_action => 'BEGIN
                                        HBG_PROCESS_ORDERS_PKG.SEND_ORDER_ATTACHMENTS('''
                                                                                                                               || p_batch_id
                                                                                                                               || ''','''
                                                                                                                               || p_password
                                                                                                                               || ''','''
                                                                                                                               ...
                                                                                                                               || ''');
                                    END;', enabled => true, auto_drop => true,
                                 comments => 'HBG Send Order Attachment Batch ' || p_batch_id);

        l_tracker.batch_id := p_batch_id;
        l_tracker.status := 'NEW';
        l_tracker.comments := NULL;
        l_tracker.CREATION_DATE := sysdate;
        INSERT INTO HBG_PROCESS_ORDERS_ATTACHMENT_JOB_TRACKER VALUES l_tracker;
        ...
        UPDATE HBG_PROCESS_ORDERS_ADDRESSES_FBDI adr
            SET PARTY_SITE_ID = (SELECT  hzps.party_site_id
                                    ...
                                    WHERE   hzcasa.CUST_ACCOUNT_ID = adr2.CUSTOMER_ID
                                        AND hzcasa.party_site_id     = hzps.party_site_id
                                        AND hzcasa.SHIP_TO_FLAG = 'P'
                                        AND adr2.SOURCE_TRANSACTION_ID = adr.SOURCE_TRANSACTION_ID
                                        AND adr2.SBI_UUID = adr.SBI_UUID
                                        AND adr2.ADDRESS_USE_TYPE = 'BILL_TO'
                                        )               
            WHERE   SBI_UUID = p_batch_id
    ...
    /*=========================================================================*/	
        ...
        SET STATUS = 'ERROR'
        WHERE EXISTS(SELECT hdr.source_transaction_id FROM HBG_PROCESS_ORDERS_HEADERS_FBDI hdr WHERE HDR.source_transaction_id = line.source_transaction_id and 
                        hdr.STATUS = 'ERROR' AND hdr.SBI_UUID = line.SBI_UUID)
        AND SBI_UUID = p_batch_id
        AND status = 'NEW';
        ...
        SET STATUS = 'ERROR'
        WHERE EXISTS(SELECT hdr.source_transaction_id FROM HBG_PROCESS_ORDERS_HEADERS_FBDI hdr WHERE HDR.source_transaction_id = hef.source_transaction_id and 
                        hdr.STATUS = 'ERROR'  AND hdr.SBI_UUID = hef.SBI_UUID)
        AND SBI_UUID = p_batch_id
        AND status = 'NEW'; 
        ...
        SET STATUS = 'ERROR'
        WHERE EXISTS(SELECT hdr.source_transaction_id FROM HBG_PROCESS_ORDERS_HEADERS_FBDI hdr WHERE HDR.source_transaction_id = lef.source_transaction_id and 
                        hdr.STATUS = 'ERROR' AND hdr.SBI_UUID = lef.SBI_UUID)
        AND SBI_UUID = p_batch_id
        AND status = 'NEW'; 
        ...
        SET STATUS = 'ERROR'
        WHERE EXISTS(SELECT hdr.source_transaction_id FROM HBG_PROCESS_ORDERS_HEADERS_FBDI hdr WHERE HDR.source_transaction_id = tkn.source_transaction_id and 
                        hdr.STATUS = 'ERROR' AND hdr.SBI_UUID = tkn.SBI_UUID)
        AND SBI_UUID = p_batch_id
        AND status = 'NEW'; 
        ...
        SET STATUS = 'ERROR'
        WHERE EXISTS(SELECT hdr.source_transaction_id FROM HBG_PROCESS_ORDERS_HEADERS_FBDI hdr WHERE HDR.source_transaction_id = cmt.source_transaction_id and 
                        hdr.STATUS = 'ERROR' AND hdr.SBI_UUID = cmt.BATCH_ID)
        AND BATCH_ID = p_batch_id
        AND status = 'NEW'; 
        
        --UPDATE CHARGES TABLE
        dbms_output.put_line('COMMENTS ERROR ADDRESS UPDATE - ' || systimestamp);
        UPDATE HBG_PROCESS_ORDERS_CHARGES_FBDI cmt
        SET STATUS = 'ERROR'
        WHERE EXISTS(SELECT hdr.source_transaction_id FROM HBG_PROCESS_ORDERS_HEADERS_FBDI hdr WHERE HDR.source_transaction_id = cmt.source_transaction_id and 
                        hdr.STATUS = 'ERROR' AND hdr.SBI_UUID = cmt.SBI_UUID)
        AND SBI_UUID = p_batch_id
        AND status = 'NEW'; 
        
        --UPDATE CHARGES COMPS TABLE
        dbms_output.put_line('COMMENTS ERROR ADDRESS UPDATE - ' || systimestamp);
        UPDATE HBG_PROCESS_ORDERS_CHARGES_COMPS_FBDI cmt
        SET STATUS = 'ERROR'
        WHERE EXISTS(SELECT hdr.source_transaction_id FROM HBG_PROCESS_ORDERS_HEADERS_FBDI hdr WHERE HDR.source_transaction_id = cmt.source_transaction_id and 
                        hdr.STATUS = 'ERROR' AND hdr.SBI_UUID = cmt.SBI_UUID )
        AND SBI_UUID = p_batch_id
    ...
    /*=========================================================================*/
        ...
        UPDATE HBG_PROCESS_ORDERS_CHARGES_FBDI 
        SET STATUS = 'VALIDATED'
        WHERE STATUS = 'NEW'
        AND SBI_UUID = p_batch_id;
        
        UPDATE HBG_PROCESS_ORDERS_CHARGES_COMPS_FBDI 
        SET STATUS = 'VALIDATED'
        WHERE STATUS = 'NEW'
        AND SBI_UUID = p_batch_id;
    ...
    END VALIDATE_CX_DATA;
    
    
    PROCEDURE CALL_CX_ORDER_JOB (
        p_debug         IN VARCHAR2,
        p_instanceid    IN VARCHAR2,
        p_creation_date IN DATE,
        errbuf       OUT VARCHAR2,
        ...
        l_errbuf  VARCHAR2(3000);
        l_retcode VARCHAR2(1);
        l_tracker HBG_CX_ORDERS_JOB_TRACKER%rowtype;
        l_timestamp VARCHAR2(20);
    BEGIN
        errbuf := NULL;
        retcode := 0;
        ...
        job_name := 'HBG_PROCESS_ORDERS_' || l_timestamp;
        dbms_scheduler.create_job(job_name => 'HBG_PROCESS_ORDERS_' || l_timestamp, job_type => 'PLSQL_BLOCK', job_action => 'BEGIN
                                        HBG_PROCESS_ORDERS_PKG.CX_TO_STAGE_TABLES('''
                                                                                                                               || p_debug
                                                                                                                               || ''','''
                                                                                                                               || p_instanceid
                                                                                                                               || ''','''
                                                                                                                               ...
                                                                                                                               || ''');
                                    END;', enabled => true, auto_drop => true,
                                 comments => 'HBG Cx Orders to Stage Tables ' || p_instanceid);
        ...
        l_tracker.retcode := 0;
        l_tracker.CREATION_DATE := sysdate;
        INSERT INTO HBG_CX_ORDERS_JOB_TRACKER VALUES l_tracker;

...
--            maint.cust_account_id,
    ...
    BEGIN
        ...
        FOR rec IN c1 LOOP
            ...
            p_ar_custmaintain_writeoff_recs(i).je_description := rec.je_reason;
        ...
        p_ar_custmaintain_writeoff_recs(i).je_description := 'Clearing Account';
        p_ar_custmaintain_writeoff_recs(i).group_id := p_process_id;
    END;
    ...
    PROCEDURE create_refund_transaction (
        p_process_id    IN NUMBER,
        p_amount        OUT NOCOPY VARCHAR2,
        p_refund_amount OUT NOCOPY VARCHAR2,
        p_acct_number   OUT NOCOPY VARCHAR2,
        p_trx_class     OUT NOCOPY VARCHAR2,
        p_transction_id OUT NUMBER,
    ...
    BEGIN
        ...
        IF v_cnt = 1 THEN
            ...
            WHERE
                    process_id = p_process_id
                AND trx_class IN ( 'CM', 'ONACC', 'RCPT' )
            GROUP BY
                nvl(new_account_number, account_number),
                trx_class;

        ELSE
--            need to skip the transactions as number of records seleted are more than 1;
            p_count := v_cnt;
        END IF;

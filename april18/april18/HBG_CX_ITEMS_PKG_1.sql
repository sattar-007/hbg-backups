...
--------------------------------------------------------
  ...
  * 01/09/2023   Delia Nugent       HUGCX-1263 also removed hardcoding on function generatereports
  * 03/07/2023   Delia Nugent       commented out references to HBG_CX_ITEMS_TEMP 
  * 03/13/2023   Delia Nugent       HUGCX-1316 
  ************************************************************************/
  ...
  --*************************************************************************/
    ...
    l_last_updated_by  hbg_cx_items_extract.last_updated_by%TYPE ; 
    BEGIN
        errbuf := NULL;
        retcode := 0;
        ...
        UPDATE hbg_cx_items_extract item SET status = 'SKIPPED', cx_active = 'false', comments = 'SKIPPED: Owner code ['
                                  || item.owner_code
                                  || '] not valid;',
                                  last_updated_by = l_last_updated_by||'1'
        /* HUGCX-1263 */
        WHERE NOT EXISTS (SELECT 1 FROM hbg_cx_common_lookups c
        WHERE lookup_type = 'CX_VALID_OWNER_CODE'
        AND item.owner_code = c.lookup_code
        ...
        UPDATE HBG_CX_ITEMS_EXTRACT item SET STATUS = 'SKIPPED', CX_ACTIVE = 'false', COMMENTS = COMMENTS || 'SKIPPED: Null Publication Date;',
         last_updated_by = l_last_updated_by||'2'
        ...
        UPDATE HBG_CX_ITEMS_EXTRACT item SET STATUS = 'SKIPPED', CX_ACTIVE = 'false', COMMENTS = COMMENTS || 'SKIPPED: Invalid Format ['
                                  || item.format
                                  || '];',
                                    last_updated_by = l_last_updated_by||'3'
        ...
        UPDATE HBG_CX_ITEMS_EXTRACT item SET STATUS = 'SKIPPED', CX_ACTIVE = 'false', COMMENTS = COMMENTS || 'SKIPPED: Invalid Sub Format ['
                                  || item.sub_format
                                  || '];',
                                          last_updated_by = l_last_updated_by||'4'
        ...
        UPDATE HBG_CX_ITEMS_EXTRACT item SET STATUS = 'SKIPPED', CX_ACTIVE = 'false', COMMENTS = COMMENTS || 'SKIPPED: Invalid Publication Status Code ['
                                  || item.pub_status
                                  || '];',
                                          last_updated_by = l_last_updated_by||'5'
                WHERE item.pub_status IN ( 'NOP', 'PC', 'OP', 'OSI', 'OSF' )
                AND INSTANCEID = p_instance_id
                --AND NOT EXISTS (SELECT 1 FROM HBG_CX_ITEMS_PRODUCTS_SKUS cx WHERE cx.sku_id = item.isbn)
                ;
        COMMIT;
         UPDATE HBG_CX_ITEMS_EXTRACT item SET STATUS = 'NEW', COMMENTS = NULL,        last_updated_by = l_last_updated_by||'6'
      ...
      --   and HIDE_FROM_ONIX = 'Y' -- HUGCX-1263
         and owner_code <> 'PK' -- 11/4 
         and format_code <> '50' -- 11/7 
         AND INSTANCEID = p_instance_id;
         COMMIT; 

         */
        UPDATE HBG_CX_ITEMS_EXTRACT SET STATUS = 'NEW', COMMENTS = NULL, CX_ACTIVE = 'true' ,        last_updated_by = l_last_updated_by||'7'
        where isbn in (SELECT ISBN FROM HBG_CX_ITEMS_HOLIDAY_ITEMS)
                and instanceid = p_instance_id;
        COMMIT; -- 11/4

         UPDATE HBG_CX_ITEMS_EXTRACT item SET CX_ACTIVE = 'false',        last_updated_by = l_last_updated_by||'8'
         WHERE CX_ACTIVE IS NULL
         and pub_status = 'NYP'
         ...
         UPDATE HBG_CX_ITEMS_EXTRACT item SET CX_ACTIVE = 'false',        last_updated_by = l_last_updated_by||'9'
         WHERE CX_ACTIVE IS NULL
         and CUSTOMER_SPECIFIC_CODE IS NOT NULL
         AND INSTANCEID = p_instance_id;
         COMMIT; 

         UPDATE HBG_CX_ITEMS_EXTRACT item SET CX_ACTIVE = 'true',        last_updated_by = l_last_updated_by||'10'
         WHERE CX_ACTIVE IS NULL
         AND INSTANCEID = p_instance_id;
         COMMIT;    
        ...
        UPDATE hbg_cx_items_bisac_extract bisac
            ...
            WHERE
                instanceid = p_instance_id
                AND EXISTS (SELECT 1 FROM HBG_CX_ITEMS_EXTRACT item WHERE item.isbn = bisac.isbn and (item.status = 'SKIPPED'
                --or item.owner_code not in ('CB', 'AB', 'QS') -- /*HUGCX-1263 */
                )
                );
        COMMIT;
  ...
  --*************************************************************************/
    ...
    BEGIN
        SELECT meaning
        INTO l_debug
        FROM hbg_cx_common_lookups
        WHERE lookup_type = 'DEBUG_MODE'
        AND lookup_code = 'DEBUG_FLAG';
    EXCEPTION
    ...
    -- tchaco
    --update hbg_cx_items_execution_tracker set request = 'Import SKUS: {"product": [' where instanceid = p_instance_id;
        FOR rws IN (
            SELECT
                item.work_isbn,                 
                item.isbn                      AS sku_id,               
            ...
            --dbms_output.put_line('import_skus - SKU: ' || l_rws_sku);
            --dbms_output.put_line('import_skus - BEGIN Get Authors List - SKU: ' || l_rws_sku);
            BEGIN
                SELECT DISTINCT
                    CASE
            ...
            EXCEPTION
                WHEN no_data_found THEN
                    l_author_list := NULL;
                WHEN OTHERS THEN
                    dbms_output.put_line('Import SKU - Get Author List ERROR - ' || sqlerrm);
                    ...
                    COMMIT;
            END;
            ----dbms_output.put_line('import_skus - END Get Authors List - SKU: ' || l_rws_sku);

            IF l_author_exception = 0 THEN

            --dbms_output.put_line('import_skus - BEGIN Validate Mirror Table - SKU: ' || l_rws_sku);
            BEGIN
                SELECT
                    item.isbn
                INTO l_isbn
            ...
            EXCEPTION
                ...
                WHEN OTHERS THEN
                    ...
                    RAISE failed_import;
            END;
            --dbms_output.put_line('import_skus - END Validate Mirror Table - SKU: ' || l_rws_sku);
            ...
            IF l_isbn IS NULL THEN
                ...
                IF  rws.update_flag = 'false' then
                    l_media := ',"productImages":[{"path":"/products/'||rws.owner_code||'.jpg", "name":"'||rws.owner_code||'.jpg"}]';
                ELSE   
                    l_media := NULL;
                END IF;
                ...
                IF l_first_rec > 0 THEN
                    l_skus_request := ',{"id" : "'
                                      ...
                                      || l_media
                                      || ',"listPrices": {"giftingPrice": 0.0,"canadianPriceGroup": 0.0,"giftingPriceCAD": 0.0,"defaultPriceGroup": 0.0, "employeePriceCAD" : 0.0, "employeePriceUSD" : 0.0}'
                                      || ', "childSKUs": [{"id" : "'
                                      || rws.sku_id
                                      || '","displayName": "'
                                      || REPLACE(rws.displayname,'"','\"')
                                      ...
                                      || REPLACE(rws.imprint,'"','\"')
                                      || '","x_longDescription" : "'
                                      || REPLACE(REPLACE(rws.long_desc,'"','\"'),chr(9),'')
                                      || '","x_owner" : "'
                                      || REPLACE(rws.owner,'"','\"')
                                      || '","x_reportingGroup" : "'
                       ...
                       --dbms_output.put_line('SKU JSON Load File - NOT First Rec: ' || l_skus_request);
                    ELSE 
                        l_skus_request := '{"id" : "'
                                      || rws.sku_id
                                      || '","displayName": "'
                                      ...
                                      || '","excludeFromSitemap": false'
                                      || l_media
                                      -- tchaco || ',"listPrices": {"giftingPrice": 0.0,"canadianPriceGroup": 0.0,"giftingPriceCAD": 0.0,"defaultPriceGroup": 0.0}'
                                      || ',"listPrices": {"giftingPrice": 0.0,"canadianPriceGroup": 0.0,"giftingPriceCAD": 0.0,"defaultPriceGroup": 0.0, "employeePriceCAD" : 0.0, "employeePriceUSD" : 0.0}'
                                      || ', "childSKUs": [{"id" : "'
                                      || rws.sku_id
                                      || '","displayName": "'
                                      ...
                                      ||'"}]}';
                        l_first_rec := 1;
                        --dbms_output.put_line('SKU JSON Load File - First Rec: ' || l_skus_request);
                        END IF;
                ...
                END;*/
                    ...
                    --dbms_output.put_line('SKU JSON Load File - BEGIN WRITE IN FILE');
                    --dbms_output.put_line(DBMS_LOB.GETLENGTH(l_skus_request));
                    --
                    utl_file.put_line(l_sku_csv_file,l_skus_request);
                    --dbms_output.put_line('SKU JSON Load File - END WRITE IN FILE');
                END IF;
                END IF;
                END IF;
                end loop;
        ...
        IF l_call_api = 1 THEN
            ...
            END IF;
        /*
       INSERT INTO HBG_CX_ITEMS_TEMP (BLOB, INSTANCEID, PROCESS)
        VALUES
        (l_file_blob,
         p_instance_id,
         'skus');
        ...
        -----TEST FILES SCRIPT------
           -- teste := utl_file.fopen(g_directory, l_collections_filename, 'R');
            ...
            IF l_file_token IS NULL THEN
                ...
                RAISE failed_import;
            END IF;
            --dbms_output.put_line('Import SKUs - POST on /ccadmin/v1/files - Token = ' || l_file_token);

            --dbms_output.put_line('import_skus - BEGIN execute_bulk_import_api');
            l_process_id := execute_bulk_import_api(l_access_token, 'ProductsV2');
            --dbms_output.put_line('import_skus - END execute_bulk_import_api');
            --dbms_output.put_line('Import SKUs -  POST on /ccadmin/v1/importProcess - Token = ' || l_process_id);

            IF l_process_id IS NULL THEN
                errbuf := 'Failed to execute the import process';
                RAISE failed_import;
            END IF;
            --update hbg_cx_items_execution_tracker set process_id = l_process_id where instanceid = p_instance_id;
            --commit;

            --dbms_output.put_line('import_skus - BEGIN get_import_process_api');
            ...
            --dbms_output.put_line('import_skus - END get_import_process_api');
           ...
           INSERT INTO hbg_cx_items_json_report
                ...
                BEGIN 
                    --dbms_output.put_line('Import SKU - Update Status to ERROR_SKU with json.comments');
                    --
                    UPDATE hbg_cx_items_extract item
                    SET STATUS = STATUS || (SELECT 'ERROR_SKU'
                                                FROM hbg_cx_items_failed_rec  failed_records
                                                WHERE failed_records.product_id = item.isbn
                                                     AND failed_records.instanceid = p_instance_id
                                                     AND rownum = 1 ), -- HUGCX-1316
                        comments = comments || (
                            SELECT DISTINCT
                                Substr('ERROR_SKU - '
                                || LISTAGG(json.comments,',' ) within group (order by json.comments),1,4000)   -- HUGCX-1316
                            FROM
                ...
                --dbms_output.put_line('import_skus - UPDATE/CREATE mirror table');
                --UPDATE MIRROR TABLE
                begin 
    ...
    EXCEPTION
        WHEN failed_import THEN
            -- 
            --dbms_output.put_line('import_skus - FAILED IMPORT: ' || errbuf);
            retcode := 1;
            UPDATE hbg_cx_items_extract
            SET
                comments = comments
        ...
        WHEN OTHERS THEN
            errbuf := sqlerrm;
            --dbms_output.put_line('import_skus - exception OTHERS: ' || errbuf);
  ...
  PROCEDURE import_related_items (
    ...
    --Create Skus Json file
        ...
        -- TCHACO
        --update hbg_cx_items_execution_tracker set request = request || 'Import Related Items: {"product": [' where instanceid = p_instance_id;
        commit;
        ...
        IF l_file_token IS NULL THEN
            ...
            RAISE failed_import;
        END IF;
        --dbms_output.put_line('Related Items - POST on /ccadmin/v1/files - Token = ' || l_file_token);

        --dbms_output.put_line('import related items - BEGIN execute_bulk_import_api');
        l_process_id := execute_bulk_import_api(l_access_token, 'ProductsV2');
        --dbms_output.put_line('import related items - END execute_bulk_import_api');
        --dbms_output.put_line('Related Items - POST on /ccadmin/v1/importProcess - Token = ' || l_process_id);
        ...
        --dbms_output.put_line('import related items - BEGIN get_import_process_api');
            get_import_process_api(p_user_token => l_access_token, p_process_id => l_process_id, l_json_report_clob => l_json_report_clob,
            l_failed_records_clob => l_failed_records_clob, l_failure_count => l_failure_count,
                                  errbuf => l_errbuf, retcode => l_retcode);  
        --dbms_output.put_line('import related items - END get_import_process_api');
        ...
        IF l_failure_count > 0 THEN
                ...
                COMMIT;
                retcode := 2;
                --dbms_output.put_line('Import Related Items - Update Status to ERROR_RELATED_ITEMS with json.comments');
                BEGIN
                    UPDATE hbg_cx_items_extract item
                    SET STATUS = STATUS || (select 'ERROR_RELATED_ITEMS'
                                                from hbg_cx_items_failed_rec  failed_records
                                                where failed_records.product_id = item.isbn
                                                AND failed_records.instanceid = p_instance_id),
                        comments = comments || (
    ...
    EXCEPTION
        WHEN failed_import THEN
            retcode := 1;
            --dbms_output.put_line('Import Related Items - FAILED IMPORT: ' || errbuf);
        ...
        WHEN OTHERS THEN
            errbuf := sqlerrm;
            --dbms_output.put_line('Import Related Items - OTHER EXCEPTION: ' || errbuf);
            retcode := 1;
            UPDATE hbg_cx_items_extract
            SET
                comments = comments
  ...
  --*************************************************************************/
    ...
    COMMIT;*/
        ...
        --Create Skus Json file
            l_prices_json_file := utl_file.fopen(g_directory, l_prices_filename, 'W', 32767);
        --skus loop
            FOR rws IN (
                SELECT DISTINCT
                    item_code                                                                 AS sku_id,
                    ...
                    nvl(rtrim(to_char(round(price.usd_msrp * (1-dis.discount), 2), 'FM999999999990.99'), '.'), 0) AS usd_discount_price,
                    nvl(rtrim(to_char(round(price.cad_msrp * (1-dis.discount), 2), 'FM999999999990.99'), '.'), 0) AS cad_discount_price
                FROM
                    hbg_cx_items_price_extract price,
                    hbg_cx_items_extract item,
            ...
            --dbms_output.put_line(rws.sku_id);
                IF countrws = 'true' THEN
                    l_price_request := '{"price" : [{"productId" : "'
                                       ...
                                       || '","skuId" : "'
                                       || rws.sku_id
                                       || '","priceListId" : "employeePriceCAD_salePrices","listPrice" : '
                ...
                ELSE
                    ...
                    utl_file.put_line(l_prices_json_file, l_price_request);
                END IF;
                --dbms_output.put_line(l_price_request);
                --update hbg_cx_items_execution_tracker set request = request || l_price_request where instanceid = p_instance_id;
                --commit;
            END LOOP;
           ...
           --INSERT INTO HBG_CX_ITEMS_TEMP (BLOB, INSTANCEID, PROCESS) VALUES (l_prices_blob,p_instance_id, 'PRICES' );
            COMMIT;
            --get cx api access token
            l_access_token := get_api_access_token;
            IF l_access_token IS NULL THEN
   ...
   /* IF retcode = 1 THEN
    ...
    EXCEPTION
        WHEN failed_import THEN
            --dbms_output.put_line('Import Price - Failed Import - ' || sqlerrm);
            retcode := 1;
            UPDATE hbg_cx_items_price_extract
            SET
                status = 'ERROR',
        ...
        WHEN OTHERS THEN
            errbuf := sqlerrm;
            --dbms_output.put_line('Import Price - OTHER EXCEPTIONS - ' || errbuf);
            retcode := 1;
            UPDATE hbg_cx_items_price_extract
            SET
                status = 'ERROR',
  ...
  --*************************************************************************/
    ...
    BEGIN
     ...
     --get cx api access token
        ...
        -- tchaco
        --update hbg_cx_items_execution_tracker set request = request || 'Import Collections: {"product": [' where instanceid = p_instance_id;
        --commit;
       --dbms_output.put_line('access_token=' || l_access_token);
       l_prd_count := 0;

       BEGIN
        --dbms_output.put_line('Import Collections - begin delete inactive collections');
        ...
        --dbms_output.put_line('Import Collections - end delete inactive collections');
    ...
    --collections loop
        FOR prd IN (
            ...
            WHERE
                ...
                AND EXISTS (SELECT 1 FROM hbg_cx_common_lookups c
                WHERE LOOKUP_TYPE = 'allGifting_OWNER_CODE'
                AND item.owner_code = c.lookup_code
                AND c.ENABLED_FLAG = 'Y' ) 
                /* HUGCX-1263 */
                AND publication_date IS NOT NULL
                and (CUSTOMER_SPECIFIC_CODE IS NULL or (format_code = '50'))
                and ((item.pub_status = 'NYP' and to_date(item.publication_date,'YYYY-MM-DD') > trunc(sysdate))
                    OR item.pub_status <> 'NYP')
                /* HUGCX-1263    
                AND ( 
                --item.HIDE_FROM_ONIX = 'N' or item.HIDE_FROM_ONIX is NULL 
                 --   or  (format_code = '50'))
                            HUGCX-1263*/
            ...
            WHERE
                            --item.status = 'NEW'
                    item.instanceid = p_instance_id
                --AND item.HIDE_FROM_ONIX <> 'Y'
                AND pub_status NOT IN ( 'NOP', 'PC', 'OP', 'OSI', 'OSF' )
                --AND OWNER_CODE IN ('HB','CB','HU','OB','MS','AB','PP','GM','DP','HL','QS','KP','LP','YP')
                AND publication_date IS NOT NULL
                and (CUSTOMER_SPECIFIC_CODE IS NULL or (format_code = '50'))
                /* HUGCX-1263 
                AND ( item.HIDE_FROM_ONIX = 'N' or item.HIDE_FROM_ONIX is NULL 
                    or format_code = '50'))
                                    HUGCX-1263  */   
                and ((item.pub_status = 'NYP' and to_date(item.publication_date,'YYYY-MM-DD') > trunc(sysdate))
                    OR item.pub_status <> 'NYP')
     ...
     from   hbg_cx_items_bisac_extract bisac,
        ...
        and item.PUB_STATUS NOT IN ('NOP','PC','OP','OSI','OSF')
                /* HUGCX-1263 */
                AND EXISTS (SELECT 1 FROM hbg_cx_common_lookups c
                WHERE LOOKUP_TYPE = 'OWNER_SPECIFIC_CAT'
                AND item.owner_code = c.lookup_code
                AND c.ENABLED_FLAG = 'Y' ) 
                /* HUGCX-1263 */
     ...
     from   hbg_cx_items_bisac_extract bisac,
        ...
        where bisac.isbn = item.isbn
                and item.instanceid = p_instance_id
           --     AND (item.HIDE_FROM_ONIX = 'N'or item.HIDE_FROM_ONIX is null) /* HUGCX-1263 */
                and CUSTOMER_SPECIFIC_CODE IS NULL
                and bisac.instanceid = p_instance_id
                AND col.product_id (+) = item.isbn
     ...
     from   
            hbg_cx_items_extract item,
            HBG_CX_ITEMS_HOLIDAY_ITEMS hol,
            hbg_cx_items_categories col


        where 
                item.instanceid = p_instance_id
            and item.isbn = hol.isbn
            AND col.product_id (+) = item.isbn
            AND col.category_id (+) = hol.ID
        and item.PUB_STATUS NOT IN ('NOP','PC','OP','OSI','OSF')
                AND item.PUBLICATION_DATE IS NOT NULL
                AND item.FORMAT_CODE NOT IN (
     ...
     --  dbms_output.put_line('Import Collections - PRODUCT ID = ' || prd.product_id); 
        ...
        if l_col_count > 0 then
         ...
         -- insert into hbg_cx_items_temp (blob, process, instanceid) values (l_collections_blob, 'COLLECTIONS',p_instance_id ); COMMIT;
           --dbms_output.put_line('ACCESS TOKEN INIT');
            ...
            IF l_file_token IS NULL THEN
                ...
                RAISE failed_import;
            END IF;
            --dbms_output.put_line('Import Collections - POST on /ccadmin/v1/files - Token = ' || l_file_token);
           ...
           -- execute bulk import 
            ...
            IF l_process_id IS NULL THEN
                ...
                RAISE failed_import;
            END IF;
            --dbms_output.put_line('Import Collections -  POST on /ccadmin/v1/importProcess - Token = ' || l_process_id);
            ...
            IF l_failure_count > 0 THEN
                ...
                --dbms_output.put_line('Import Collections - Update Status to ERROR_COLLECTIONS with json.comments');                

                BEGIN
                    UPDATE hbg_cx_items_extract item
                    SET STATUS = STATUS || (select 'ERROR_COLLECTIONS' from
                        ...
                        comments = comments || (
                            SELECT DISTINCT
                                'ERROR_COLLECTIONS - '
                                || case when json.comments like '%List price must be a positive number or 0%' then 'Product ID ['|| failed_records.product_id || '] is not on CX.'
                                else json.comments end as comments
                            FROM
                                hbg_cx_items_col_failed_rec  failed_records,
                    ...
                    UPDATE hbg_cx_items_bisac_extract bisac
                    SET
                        status = (select 'ERROR' from 
                                    hbg_cx_items_col_failed_rec  failed_records
                                    where failed_records.product_id = bisac.isbn
                                AND failed_records.instanceid = p_instance_id),
    ...
    EXCEPTION
        WHEN failed_import THEN
            ...
            --dbms_output.put_line('Import Collections - FAILED IMPORT: ' || errbuf);
        ...
        WHEN OTHERS THEN
            errbuf := sqlerrm;
            --dbms_output.put_line('Import Collections - OTHER EXCEPTION: ' || errbuf);
            retcode := 1;
            UPDATE hbg_cx_items_extract
            SET
                comments = comments
  ...
  --*************************************************************************/
    ...
    BEGIN
        ...
        --dbms_output.put_line('BEGIN - item validation');
        isbn_validation(p_instanceid, l_errbuf, l_retcode);
        --dbms_output.put_line('END - item validation');
        IF l_retcode = 1 THEN
        ...
        --dbms_output.put_line('BEGIN - import sku');
        import_skus(p_instanceid, p_auth, l_errbuf, l_retcode);
        --dbms_output.put_line('END - import sku');
        IF l_retcode > 0 THEN
            errbuf := errbuf
                      || 'Skus : '
                      || l_errbuf
        ...
        COMMIT;
         ...
         my_time := systimestamp;
         --CALL IMPORT SKU PROCEDURE
         --dbms_output.put_line('BEGIN - import related items');
        import_related_items(p_instanceid, p_auth, l_errbuf, l_retcode);
        --dbms_output.put_line('END - import related items');
        IF l_retcode > 0 THEN
            errbuf := errbuf
                      || 'Related_Items : '
                      || l_errbuf
     ...
     --  dbms_output.put_line('BEGIN - import collections');
        import_collections(p_instanceid, p_auth, l_errbuf, l_retcode);
      --  dbms_output.put_line('END - import collections');
        IF l_retcode > 0 THEN
            errbuf := errbuf
                      || 'Collections : '
                      || l_errbuf
        ...
        --dbms_output.put_line('BEGIN - import prices');
        import_prices(p_instanceid, p_auth, l_errbuf, l_retcode);
        --dbms_output.put_line('END - import prices');
        IF l_retcode > 0 THEN
            errbuf := errbuf
                      || 'Prices : '
                      || l_errbuf
        ...
        my_time := systimestamp;
        --dbms_output.put_line('BEGIN - import inventory');
        import_inventory(p_instanceid, p_auth, l_errbuf, l_retcode);
        --dbms_output.put_line('END - import inventory');
        IF l_retcode > 0 THEN
            errbuf := errbuf
                      || 'Inventory : '
                      || l_errbuf
...
BLOB */
    ...
    BEGIN
        ...
        INTO i_user
        FROM hbg_cx_common_lookups
        WHERE lookup_type = 'FTP_CONNECT'
            AND lookup_code = 'I_USER';

        SELECT meaning
        INTO i_host
        FROM hbg_cx_common_lookups
        WHERE lookup_type = 'FTP_CONNECT'
            AND lookup_code = 'I_HOST';

        SELECT meaning
        INTO i_port
        FROM hbg_cx_common_lookups
        WHERE lookup_type = 'FTP_CONNECT'
            AND lookup_code = 'I_PORT'; 
         /* HUGCX-1263 */
        I_TRUST_SERVER := TRUE;
        ----------------ITEM EXTRACT REPORT ----------------------------------------------
        ...
        END LOOP;
        --
        utl_file.put(l_products_report,'HIDE_FROM_ONIX|CUSTOMER_SPECIFIC_CODE|CUSTOMER_SPECIFIC_DESC|STATUS|ERROR_TEXT|OIC_PARENT_ID');
  ...
  ----dbms_output.put_line('after check post publish status');
    ...
    EXCEPTION
        WHEN OTHERS THEN
            --dbms_output.put_line('ERROR on execute_bulk_import_api: ' || sqlerrm);
            RETURN NULL;
    ...
    BEGIN
        ...
        /*BEGIN  
            dbms_output.put_line('Print CLOB - start_file_upload_api');
            loop  
            exit when l_offset > dbms_lob.getlength(l_response_clob);  
            dbms_output.put_line( dbms_lob.substr( l_response_clob, 255, l_offset ) );  
            l_offset := l_offset + 255;  
    ...
    EXCEPTION
        WHEN OTHERS THEN
            --dbms_output.put_line('ERROR on start_file_upload_api: ' || sqlerrm);
            RETURN NULL;
    ...
    BEGIN
        ...
        --dbms_output.put_line('get_import_process_api - completed: ' || l_completed);
        FOR l_rec IN (
            SELECT
                tk.rel,
                tk.href
    ...
    EXCEPTION
        WHEN OTHERS THEN
            --dbms_output.put_line('ERROR on get_import_process_api - ' || sqlerrm);
            errbuf := sqlerrm;
            retcode := 1;
    ...
    PROCEDURE upload_extract_tables_cx(p_instance_id IN NUMBER,
                                       p_hbg_process_id IN NUMBER, 
                                       errbuf  OUT VARCHAR2,
                                       retcode OUT VARCHAR2
                                       ) IS
    CURSOR c_cpm IS
     SELECT * 
      ...
      WHERE hpc.owner_code IN ('HB','CB','HU','OB','MS','AB','PP','DP' ,'QS','KP','LP','YP')
        AND hpc.pub_status NOT IN ('NOP', 'PC','OP', 'OSI', 'OSF')
        AND hpc.hbg_process_id = p_hbg_process_id
        AND NOT EXISTS (SELECT 1
                          FROM HBG_CX_ITEMS_EXTRACT HCIE
                         WHERE HCIE.hbg_process_id = hpc.hbg_process_id
                           AND HCIE.isbn = hpc.isbn13);
    ...
    -- Table Types
    TYPE lt_hbg_cpm_tb IS TABLE OF c_cpm%ROWTYPE;
    TYPE lt_hbg_pbc_tb IS TABLE OF c_pbc%ROWTYPE;
    ...
    lv_hbg_pim_cpm_stg_tb lt_hbg_cpm_tb;
    lv_hbg_pim_pbc_stg_tb lt_hbg_pbc_tb;
    lv_isbn13 hbg_pim_cpm.isbn13%type;
    lv_work_isbn hbg_pim_cpm.work_isbn%type;
    lv_usd_msrp hbg_pim_cpm.usd_msrp%type; 
    lv_cad_msrp hbg_pim_cpm.cad_msrp%type; 

    --lv_instance_id NUMBER := p_instance_id;
    ...
    BEGIN
     ...
     LOOP
      ...
      FETCH c_cpm BULK COLLECT INTO lv_hbg_pim_cpm_stg_tb LIMIT lc_bulk_collect_limit;
      EXIT WHEN lv_hbg_pim_cpm_stg_tb.COUNT = 0;
         ...
         BEGIN
               ...
               FORALL i IN 1 .. lv_hbg_pim_cpm_stg_tb.COUNT SAVE EXCEPTIONS
                 ...
                 lv_isbn13 := lv_hbg_pim_cpm_stg_tb(i).isbn13;
                 lv_work_isbn lv_hbg_pim_cpm_stg_tb(i).work_isbn;
                 lv_usd_msrp lv_hbg_pim_cpm_stg_tb(i).usd_msrp;
                 lv_cad_msrp lv_hbg_pim_cpm_stg_tb(i).cad_msrp;
               
                  --INSERT PRODUCTS SKUS
                  INSERT INTO HBG_CX_ITEMS_EXTRACT
                     (isbn 
                     ,isbn10
                     ,work_isbn
                     ,work_title
                     ...
                     ,last_updated_by
                     ,hide_from_onix
                     ,customer_specific_code --check
                     ,customer_specific_desc --check 
                     ,column1            --check   
                     ,hbg_process_id
                     ,instanceid
                     )
              ...
              COMMIT;
                  ...
                  --INSERT prices
                 INSERT INTO HBG_CX_ITEMS_PRICE_EXTRACT
                     (item_code       
                     ,work_isbn       
                     ,usd_msrp        
                     ,cad_msrp        
               ...
               EXCEPTION
                WHEN OTHERS THEN
                 retcode := 1;
                 errbuf := 'ERROR when inserting records into [HBG_CX_ITEMS_EXTRACT - HBG_CX_ITEMS_PRICE_EXTRACT] - '|| SQLERRM;
                 RAISE le_insert_exception;
              END;
     ...
     LOOP
      ...
      FETCH c_pbc BULK COLLECT INTO lv_hbg_pim_pbc_stg_tb LIMIT lc_bulk_collect_limit;
      EXIT WHEN lv_hbg_pim_pbc_stg_tb.COUNT = 0;
      
         BEGIN
               FORALL i IN 1 .. lv_hbg_pim_pbc_stg_tb.COUNT SAVE EXCEPTIONS
                  --INSERT BISAC
                  INSERT INTO HBG_CX_ITEMS_BISAC_EXTRACT
                     (isbn              
                     ,bisac_sequence    
                     ,genbisac_code     
                     ,genbisac_name     
            ...
            EXCEPTION
               WHEN OTHERS THEN
                retcode := 1;
                errbuf := 'ERROR when inserting records into [HBG_CX_ITEMS_BISAC_EXTRACT] - '|| SQLERRM;
                RAISE le_insert_exception;
            END;
         END LOOP;
         --

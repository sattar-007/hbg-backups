...
-- Schema: HBG_INTEGRATION  Date: Tue Apr 18 08:29:39 2023 
--
...
BEGIN
  ...
  ORDS.DEFINE_HANDLER(
      ...
      p_source         => 
'BEGIN
hbg_ar_cust_maintenance.create_writeoff_records(p_data => :body,p_status => :status);
END;');
...
and orcl_status = (case when :p_trx_number is null then orcl_status else ''Success'' end)');
  ...
  ORDS.DEFINE_TEMPLATE(
      p_module_name    => 'ARCustomerMaintenance',
      p_pattern        => 'getReceivableActivities',
      p_priority       => 0,
      p_etag_type      => 'HASH',
      p_etag_query     => NULL,
      p_comments       => NULL);
  ...
  ORDS.DEFINE_HANDLER(
      ...
      p_comments       => NULL,
      p_source         => 
'select * from AR_RECEIVABLES_ACTIVITY where name not like ''%Clearing%''  and seg_name <> ''131806''');
...
');
  ...
  ORDS.DEFINE_HANDLER(
      ...
      p_comments       => NULL,
      p_source         => 
'Select distinct Account_number,Account_name,cust_account_id from hz_cust_Accounts hca where
(:party_id is null or hca.party_id = :party_id) 
and (:p_acct_name is null or upper(hca.account_name)  LIKE upper(:p_acct_name || ''%''))');

  ORDS.DEFINE_TEMPLATE(
      p_module_name    => 'Common',
      p_pattern        => 'getOrgNumByBame',
      p_priority       => 0,
      p_etag_type      => 'HASH',
      p_etag_query     => NULL,
      p_comments       => NULL);
...
AND PARTY_NAME = :PARTY_NAME');
  ...
  ORDS.DEFINE_HANDLER(
      ...
      p_comments       => NULL,
      p_source         => 
'Select distinct  account_number,cust_account_id, Account_name from hz_cust_Accounts where account_name = :p_account_name');
...
');
  ...
  ORDS.DEFINE_HANDLER(
      ...
      p_comments       => NULL,
      p_source         => 
'Select distinct  account_number,cust_account_id, Account_name from hz_cust_Accounts where account_number = :p_account_number');
...
'SELECT GEOGRAPHY_CODE CITY FROM HZ_GEOGRAPHIES WHERE GEOGRAPHY_TYPE = ''CITY'' and (:city is null or upper(GEOGRAPHY_CODE) LIKE upper(:city || ''%'')) ORDER BY GEOGRAPHY_CODE');
  ...
  ORDS.DEFINE_HANDLER(
      ...
      p_comments       => NULL,
      p_source         => 
'SELECT DISTINCT GEOGRAPHY_NAME "POSTAL CODE" FROM HZ_GEOGRAPHIES WHERE GEOGRAPHY_TYPE = ''POSTAL_CODE'' and (:postal is null or upper(GEOGRAPHY_NAME) LIKE upper(:postal || ''%'')) ORDER BY GEOGRAPHY_CODE

');

  ORDS.DEFINE_TEMPLATE(
      p_module_name    => 'Common',
      p_pattern        => 'getAccountNumberByName',
      p_priority       => 0,
      p_etag_type      => 'HASH',
      p_etag_query     => NULL,
      p_comments       => NULL);
...
'select deptno from emp');
  ...
  ORDS.DEFINE_TEMPLATE(
      p_module_name    => 'CustomeOrders',
      p_pattern        => 'getTemplatenameForRules',
      p_priority       => 0,
      p_etag_type      => 'HASH',
      p_etag_query     => NULL,
      p_comments       => NULL);
...
'select
 ...
 where         1 = 1
  /*  AND (
 ( :P_CUST_ACCOUNT_ID IS NULL
            OR CUST_ACCOUNT_ID = :P_CUST_ACCOUNT_ID )
          AND ( upper(:P_OWNER_CODE) IS NULL
                OR upper(OWNER_CODE) = upper(:P_OWNER_CODE) )
  )
*/
  AND NVL(CUST_ACCOUNT_ID,NVL(:P_CUST_ACCOUNT_ID,-1)) = NVL(:P_CUST_ACCOUNT_ID,-1)
  AND NVL(OWNER_CODE,NVL(:P_OWNER_CODE,''#NULL'')) = NVL(:P_OWNER_CODE,''#NULL'')');
...
AND (:P_CATEGORY_NAME IS NULL OR FDC.CATEGORY_NAME = :P_CATEGORY_NAME)
ORDER BY FDC.CATEGORY_NAME');
...
'SELECT DISTINCT
    ...
    hahvse_owner.owner_code                                 owner,
    hahvse_owner.OWNER_NAME                                owner_description,
    hahle_reporting_group.reporting_grp_code                       reporting_group,
    hahle_reporting_group.REPORTING_GRP_NAME                           reporting_group_description,
    hahle_category1.category1_code                             category_1,
    hahle_category1.CATEGORY1_NAME                                 category_1_description,
    hahle_category2.category2_code                             category_2,
    hahle_category2.CATEGORY2_NAME                                 category_2_description,
...
--TO_CHAR(HDRE.END_DATE,''MM/DD/YYYY'') 
    ...
    hdre.created_by,
    to_' || 'char(hdre.last_update_date, ''MM/DD/YYYY HH24:MI:ss'') last_update_date,
    hdre.last_updated_by,
    hdre.precedence_level
FROM
    hbg_dist_rights_ext       hdre,
    xxhbg_family_code_owner               hahvse_owner,
    xxhbg_family_code_reporting_group     hahle_reporting_group,
    xxhbg_family_code_reporting_category1 hahle_category1,
    xxhbg_family_code_reporting_category2 hahle_category2,
...
WHERE
        1 = 1
    AND hahvse_owner.owner_id = hahle_reporting_group.owner_id (+)
    AND hahle_rep' || 'orting_group.reporting_group_id = hahle_category1.reporting_group_id (+)
    AND hahle_category1.category1_id = hahle_category2.category1_id (+)
	...
	AND hdre.owner = hahvse_owner.owner_code (+)
    AND hdre.reporting_group = hahvse_owner.owner_code
                               || hahle_reporting_group.reporting_grp_code (+)
    AND hdre.category_1 = hahvse_owner.owner_code
                          || hahle_reporting_group.reporting_grp_code
                          || hahle_category1.cat' || 'egory1_code (+)
    AND hdre.category_2 = hahvse_owner.owner_code
                          || hahle_reporting_group.reporting_grp_code
                          || hahle_category1.category1_code
                          || hahle_category2.category2_code (+)
    ...
    AND nvl(hahvse_owner.owner_code, ''' || '#NULL'') LIKE nvl(:p_owner, nvl(hahvse_owner.owner_code, ''#NULL''))
    AND nvl(hahle_reporting_group.reporting_grp_code, ''#NULL'') LIKE nvl(:p_reporting_group, nvl(hahle_reporting_group.reporting_grp_code, ''#NULL''))
    AND nvl(hahle_category1.category1_code, ''#NULL'') LIKE nvl(:p_category1, nvl(hahle_category1.category1_code, ''#NULL''))
    AND nvl(hahle_category2.category2_code, ''#NULL'') LIKE nvl(:p_category2, nvl(hahle_category2.category2_code, ''#NULL''))
    ...
    AND to_char(nvl(hdre.edition, 999999999)) LIKE
        CASE
            WHEN upper(:p_edition) = ''#NULL'' THEN
                ''999999999''
            ELSE
                nvl(:p_edition, nvl(hdre.edition, 999999999))
        END
    ...
    AND nvl(hca.account_number, ''A#NULL'') LIKE nvl(:p_account_number, nvl(hca.account_number, ''A#NULL''))
    ...
    AND ( decode(:p_from_pubdate, NULL, nvl(hdre.from_pub_date, sysdate), hdre.from_pub_date) >= nvl(to_date(:p_from_pubdate, ''YYYY-MM-DD''),
    nvl(hdre.from_pub_date, sysdate))
          AND decode(:p_to_pub_date, NULL, nvl(hdre.to_pub_date, sys' || 'date), hdre.to_pub_date) <= nvl(to_date(:p_to_pub_date, ''YYYY-MM-DD''),
          nvl(hdre.to_pub_date, sysdate)) )
    AND nvl(hdre.end_date, sysdate) >= CASE
                                           WHEN :p_show_only_active = ''Y'' THEN
                                               sysdate
                                           ELSE
                                               nvl(hdre.end_date, sysdate)
                                       END
ORDER BY
    distribution_right_id DESC');
...
HBG_AR_PAYMENTS_PKG.DELETE_PAYMENT_LINE_P (
    P_LINE_ID  => :P_LINE_ID,
    P_BATCH_ID  => :P_BATCH_ID,
    P_STATUS    => :P_STATUS,
    P_MESSAGE     => :P_MESSAGE
  ...
  ORDS.DEFINE_PARAMETER(
      ...
      p_method             => 'DELETE',
      p_name               => 'P_LINE_ID',
      p_bind_variable_name => 'P_LINE_ID',
      p_source_type        => 'HEADER',
      p_param_type         => 'STRING',
      p_access_method      => 'IN',
      p_comments           => NULL);
...
order by account_number');
  ...
  ORDS.DEFINE_HANDLER(
      ...
      p_comments       => NULL,
      p_source         => 
'select account_number account_number from hz_cust_accounts hca, hz_parties hp 
where hca.party_id = hp.party_id
and upper(account_number) LIKE ''%'' || upper(:p_acct_num) || ''%''
and (:p_acct_num is not null OR :p_acct_num != ''NONE'' OR :p_acct_num != ''undefined'')
order by account_number');
...
WHERE
 ...
 ( :p_pre_reservation_number IS NULL 
	...
	AND ( :p_print_number IS NULL
            OR rl.po_number = :p_print_number )		
...
,HCA.ACCOUNT_NUMBER
,HCA.ACCOUNT_NAME
,HCA.CUST_ACCOUNT_ID CUST_ACCNT_ID
,HP.PARTY_ID
...
,HP.STATE
,HP.POSTAL_CODE
,HCSU.SITE_USE_CODE SITE_PURPOSE
,HP.ADDRESS1
,HP.ADDRESS2
,HP.ADDRESS3
,HP.ADDRESS4
,NULL BILL_TO_SAN
,NULL SHIP_TO_SAN
,NULL ACCOUNT_LOOKUP1
,NULL ACCOUNT_LOOKUP2
,NULL ACCOUNT_LOOKUP3
FROM
   HZ_PARTIES HP,
   HZ_CUST_ACCOUNTS HCA,
   HZ_PARTY_SITES HPS,
...
AND HCAS.CUST_ACCT_SITE_ID=HCSU.CUST_ACCT_SITE_ID
    ...
    AND (
         ( :USECODE IS NULL  --''BILL_TO'',''SHIP_TO''
            OR HCSU.SITE_USE_CODE = :USECODE )
         AND  ( :CUSTOMER_NAME IS NULL
            OR HP.P' || 'ARTY_NAME = :CUSTOMER_NAME )
          AND ( :ACCOUNT_NUMBER IS NULL
                OR HCA.ACCOUNT_NUMBER = :ACCOUNT_NUMBER )
				AND ( :ACCOUNT_NAME IS NULL
                OR HCA.ACCOUNT_NAME = :ACCOUNT_NAME )
				AND ( :REGISTRY_ID IS NULL
                OR HP.PARTY_NUMBER = :REGISTRY_ID )
				AND ( :SITE_NUMBER IS NULL
                OR HPS.PARTY_SITE_NUMBER = :SITE_NUMBER )
				AND ( :SITE_NAME IS NULL
                OR HPS.PARTY_SITE_NAME = :SITE_NAME )
				AND ( :COUNTRY IS NULL
                OR HP.COUNTRY = :COUNTRY )
				AND ( :ADDRESS1 IS NULL
                OR HP.ADDRESS1 = :ADDRESS1 )
					AND ( :ADDRESS2 IS NULL
                OR HP.ADDRESS2 = :ADDRESS2 )
					AND ( :ADDRESS3 IS NULL
                OR HP.ADDRESS3 = :ADDRESS3 )
					AND ( :ADDRESS2 IS NULL
                OR HP.ADDRESS4 = :ADDRESS4 )
				AND ( :CITY IS NULL
                OR HP.CITY = :CITY )
				AND ( :STATE IS NULL
                OR STATE = :STATE )
				AND ( :POSTAL_CO' || 'DE IS NULL
                OR HP.POSTAL_CODE = :POSTAL_CODE )
				/*AND ( :SAN IS NULL
                OR SAN = :SAN )
				AND ( :LOOKUP IS NULL
                OR LOOKUP = :LOOKUP )*/
...
AND (:P_OWNER is null or ASSF.SF_OWNER = :P_OWNER)');
  ...
  ORDS.DEFINE_TEMPLATE(
      p_module_name    => 'SalesCreditExtension',
      p_pattern        => 'InventoryTypeMapping',
      p_priority       => 0,
      p_etag_type      => 'HASH',
      p_etag_query     => NULL,
      p_comments       => NULL);
  ...
  ORDS.DEFINE_HANDLER(
      ...
      p_comments       => NULL,
      p_source         => 
'select * from XXHBG_INV_TYPE_MAPPING_TBL
where 1=1
AND (:P_CUST_ACCOUNT_ID IS NULL OR CUST_ACCOUNT_ID = :P_CUST_ACCOUNT_ID)
AND (:P_PARTY_NUMBER IS NULL OR PARTY_NUMBER = :P_PARTY_NUMBER)');
...
p_return_status varchar2(200);
begin
hbg_customer_master_pkg.xxhbg_inv_type_mapping_tbl_create(
		p_inventory_type               => :inventory_type             
        ,p_inventory_type_preference    => :inventory_type_preference  
        ,p_owner                        => :owner                      
        ,p_owner_description            => :owner_description          
        ,p_reporting_group              => :reporting_group            
        ,p_reporting_group_description  => :reporting_group_description
...
p_return_status varchar2(200);
begin
hbg_customer_master_pkg.xxhbg_inv_type_mapping_tbl_update(
         p_inv_id 						=> :inv_id
		,p_inventory_type               => :inventory_type             
        ,p_inventory_type_preference    => :inventory_type_preference  
        ,p_owner                        => :owner                      
        ...
        ,p_category_2_description       => :category_2_description     
        ,p_format                       => :format                     
        ,p_format_description           => :format_descript' || 'ion         
        ,p_sub_format                   => :sub_format                 
        ,p_sub_format_description      => :sub_format_description     
        ,p_party_number                 => :party_number               
		,p_cust_account_id              => :cust_account_id            
...
');
  ...
  ORDS.DEFINE_MODULE(
      p_module_name    => 'ShippingSchedules',
      p_base_path      => '/ShippingSchedules/',
      p_items_per_page => 25,
      p_status         => 'PUBLISHED',
      p_comments       => NULL);
  ...
  ORDS.DEFINE_TEMPLATE(
      p_module_name    => 'ShippingSchedules',
      p_pattern        => 'ShippingScheduleTypeLov',
      p_priority       => 0,
      p_etag_type      => 'HASH',
      p_etag_query     => NULL,
      p_comments       => NULL);
  ...
  ORDS.DEFINE_HANDLER(
      ...
      p_comments       => NULL,
      p_source         => 
'select distinct flex_value shipping_schedule_type from FND_FLEX_VALUES_VL where value_category  = ''HBG_SHIPPING_SCHEDULE''');

  ORDS.DEFINE_TEMPLATE(
      p_module_name    => 'ShippingSchedules',
      p_pattern        => 'ShippingSchRelTypeLOV',
      p_priority       => 0,
      p_etag_type      => 'HASH',
      p_etag_query     => NULL,
      p_comments       => NULL);
  ...
  ORDS.DEFINE_HANDLER(
      ...
      p_comments       => NULL,
      p_source         => 
'select distinct flex_value shipping_schedule_rel_type from FND_FLEX_VALUES_VL where value_category  = ''HBG_SHIP_SCH_REL_TYPE''');

  ORDS.DEFINE_TEMPLATE(
      p_module_name    => 'ShippingSchedules',
      p_pattern        => 'ShippingScheduleTemplate',
      p_priority       => 0,
      p_etag_type      => 'HASH',
      p_etag_query     => NULL,
      p_comments       => NULL);
...
x_return_status VARCHAR2(200);
BEGIN
hbg_shipping_schedules_pkg.hbg_ss_create_ship_sch_type_template (
        p_shipping_schedule_type                 => :shipping_schedule_type                
        ,p_add_items_automatically                => :add_items_automatically               
        ,p_remove_items_automatically             => :remove_items_automatically            
        ,p_on_sale_date_editable                  => :on_sale_date_editable                 
        ,p_on_sale_date_required                  => :on_sale_date_required                 
        ,p_release_date_editable                  => :release_date_editable                 
        ,p_release_date_required                  => :release_date_required                 
        ,p_release_date_subtract_from_osd         => :release_date_subtract_from_osd        
        ,p_cutoff_date_editable                   => :cutoff_date_editable                  
        ,p_cutoff_date_required         ' || '          => :cutoff_date_required                  
        ,p_cutoff_date_subtract_from_release_date => :cutoff_date_subtract_from_release_date
        ,p_cleanup_release_date_editable          => :cleanup_release_date_editable         
        ,p_cleanup_release_date_required          => :cleanup_release_date_required         
        ,p_cleanup_release_date_subtract_from_osd => :cleanup_release_date_subtract_from_osd
        ,p_created_by                             => :created_by
        ,p_created_date                           => null                            
        ,p_last_updated_by                        => :last_updated_by
        ,p_last_updated_date                      => null                       
...
'declare
 ...
 hbg_so_mass_update_pkg.get_sales_orders (
        ...
        ,p_from_order         => :p_from_order
        ,p_to_order           => :p_to_order
        ,p_default_account_type => :p_default_account_type
        ,p_client_account_type => :p_client_account_type

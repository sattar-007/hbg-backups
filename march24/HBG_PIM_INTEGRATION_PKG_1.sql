...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "HBG_INTEGRATION"."HBG_PIM_INTEGRATION_PKG" IS
   ...
   --
   -- -----------------------------------------------------------------------------------------------------------------
   PROCEDURE update_process_tables_p(p_hbg_process_id  IN NUMBER) IS
   ...
   BEGIN
      ...
      /*lv_debug := 'TRUNCATE_ALL_FBDI_TABLES';
      BEGIN
         EXECUTE IMMEDIATE 'TRUNCATE TABLE HBG_EGP_SYSTEM_ITEMS_INTERFACE';
         EXECUTE IMMEDIATE 'TRUNCATE TABLE HBG_EGP_ITEM_REVISIONS_INTERFACE';
         EXECUTE IMMEDIATE 'TRUNCATE TABLE HBG_EGP_ITEM_CATEGORIES_INTERFACE';
         EXECUTE IMMEDIATE 'TRUNCATE TABLE HBG_EGP_ITEM_RELATIONSHIPS_INTF';
         EXECUTE IMMEDIATE 'TRUNCATE TABLE HBG_EGO_ITEM_ASSOCIATIONS_INTF';
         EXECUTE IMMEDIATE 'TRUNCATE TABLE HBG_EGO_ITEM_INTF_EFF_B';
         EXECUTE IMMEDIATE 'TRUNCATE TABLE HBG_EGO_ITEM_INTF_EFF_TL';
         EXECUTE IMMEDIATE 'TRUNCATE TABLE HBG_EGO_ITEM_REVISION_INTF_EFF_B';
         EXECUTE IMMEDIATE 'TRUNCATE TABLE HBG_EGO_ITEM_REVISION_INTF_EFF_TL';
         EXECUTE IMMEDIATE 'TRUNCATE TABLE HBG_EGO_ITEM_SUPPLIER_INTF_EFF_B';
         EXECUTE IMMEDIATE 'TRUNCATE TABLE HBG_EGO_ITEM_SUPPLIER_INTF_EFF_TL';
         EXECUTE IMMEDIATE 'TRUNCATE TABLE HBG_EGO_STYLE_VARIANT_ATTR_VS_INTF';
         EXECUTE IMMEDIATE 'TRUNCATE TABLE HBG_EGP_TRADING_PARTNER_ITEMS_INTF';
         EXECUTE IMMEDIATE 'TRUNCATE TABLE HBG_EGP_ITEM_ATTACHMENTS_INTF';
      EXCEPTION
         WHEN OTHERS THEN
            lv_aux1 := SQLERRM;
            BEGIN
               UPDATE HBG_PIM_CPM
                  SET status = 'ERROR'
                     ,error_text = 'ERROR when truncating FBDI tables - ' || lv_aux1
            ...
            EXCEPTION
               WHEN OTHERS THEN
                  log_p('ERROR'
                       ,'ERROR when updating process tables after failing to truncate FBDI tables - ' || SQLERRM);
            END;
      ...
      BEGIN
         UPDATE hbg_integration_control
          ...
          WHERE hbg_process_id = p_hbg_process_id
            AND step_number = 7;
         --
         COMMIT;
      EXCEPTION
      ...
      WHEN ge_custom_exception THEN
         ...
         EXCEPTION
            WHEN OTHERS THEN
               ROLLBACK;
               log_p('ERROR'
                    ,'ERROR when updating status of step 7 to ERROR in control table [GE_CUSTOM_EXCEPTION]. ' || SQLERRM);
         END;
         --
         log_p('ERROR'
   ...
   --
   -- -----------------------------------------------------------------------------------------------------------------
   PROCEDURE submit_update_process_tables_p(p_hbg_process_id  IN NUMBER) IS
   ...
   BEGIN
      ...
      --EXECUTE IMMEDIATE 'BEGIN HBG_PIM_INTEGRATION_PKG.UPDATE_PROCESS_TABLES_P(' || p_hbg_process_id || '); END;';
      dbms_scheduler.create_job(job_name   => 'HBG_LOAD_ITEMS_INTO_ORACLE_PIM_CALLBACK_' || TO_CHAR(p_hbg_process_id)
                               ,job_type   => 'PLSQL_BLOCK'
                               ,job_action => 'BEGIN
                                                  HBG_PIM_INTEGRATION_PKG.UPDATE_PROCESS_TABLES_P(' || p_hbg_process_id
                                              || '); END;'
                               ,enabled    => TRUE
                               ,auto_drop  => TRUE
                               ,comments   => 'HBG Load Items Information into Oracle PIM - CALLBACK - HBG Process ID = '
                                              || TO_CHAR(p_hbg_process_id)) ;
   EXCEPTION
      WHEN ge_custom_exception THEN
         log_p('ERROR'
   ...
   BEGIN
      ...
      lv_debug := 'VALIDATE_LONG_TIME_NOT_NULL_IN_HBG_PIM_CPM';
      BEGIN
      ...
      EXCEPTION
         WHEN OTHERS THEN
            ROLLBACK;
            log_p('ERROR'
                 ,'ERROR when updating HBG_PIM_CPM status to ERROR because of [LONG_TITLE IS NULL] - ' || SQLERRM);
            RAISE ge_custom_exception;
      END;
   ...
   PROCEDURE load_fbdi_tables_p IS
      ...
      CURSOR c_egp_system_items_interface IS
         SELECT hptm.load_sequence
               ...
               ,CASE
                   WHEN hptm.organization_code IN ('ITEM_MASTER', 'HBG') AND hpc.owner_code IN ('HB', 'HL', 'HU', 'XX')
                        AND hpc.abc_code IS NOT NULL THEN
                      '1'
                   ELSE
                      NULL
                END forecast_type
               ...
               ,CASE
                   WHEN hptm.organization_code IN ('ITEM_MASTER', 'HBG') AND hpc.owner_code IN ('HB', 'HL', 'HU', 'XX') THEN
               ...
               ,hpc.series_name
               ,hpc.tier_name
               ,hpc.acquiring_editor
               ,hpc.current_editor_name
               ,hpc.profit_sharing
               ,'' contract_id1
               ,'' contract_id2
               ,hpc.pub_status
               ...
               ,(SELECT CASE
                           WHEN hppbc.spcbisac_name IS NOT NULL THEN
                              hppbc.genbisac_name || '/' || hppbc.spcbisac_name
                           ELSE
                              hppbc.genbisac_name
                        END bisac
          ...
          WHERE hpc.owner_code = hptm.owner_code
            ...
            AND hptm.digital_content_flag = (SELECT NVL(MAX(hdci.digital_content_flag), 'N') -- TEMPORARY
                                              FROM hbg_digital_content_items hdci
                                             WHERE hdci.isbn = hpc.isbn13)
          ORDER BY hpc.isbn13
                  ,hptm.load_sequence;
         ...
         SELECT hpp.rowid hpp_rowid
          ...
          WHERE hpc.isbn13 = hpp.isbn13
            ...
            AND (hpp.alternate_item_code IS NOT NULL
                 OR hpp.alternate_item_type IS NOT NULL)
            -- Physical ISBNs only
            AND hpc.digital_content_flag = (SELECT NVL(MAX(hdci.digital_content_flag), 'N') -- TEMPORARY
                                              FROM hbg_digital_content_items hdci
                                             WHERE hdci.isbn = hpc.isbn13);
         ...
         SELECT hpp.rowid hpp_rowid
          ...
          WHERE hpc.isbn13 = hpp.isbn13
            ...
            AND (hpp.role_code IS NOT NULL
                 OR hpp.cntb_sequence IS NOT NULL)
            -- Physical ISBNs only
            AND hpc.digital_content_flag = (SELECT NVL(MAX(hdci.digital_content_flag), 'N') -- TEMPORARY
                                              FROM hbg_digital_content_items hdci
                                             WHERE hdci.isbn = hpc.isbn13);
      ...
      -- Data for EGO_ITEM_INTF_EFF_B with Attribute Group Code = Distribution Edition Code
      CURSOR c_ego_item_intf_eff_b_edition IS
         SELECT hpp.rowid hpp_rowid
               ,hpp.isbn13
               ,'ITEM_MASTER' organization_code
               --,'Distribution Edition Code' attribute_group_code
               ,hpp.edition_code
               ,hpp.sequence
           FROM hbg_pim_cpm hpc
               ,hbg_pim_pem hpp
         ...
         SELECT hpp.rowid hpp_rowid
               ...
               ,CASE
                   WHEN INSTR(TRIM(hpc.author_name), ' ') = 0 THEN
                      SUBSTR(TRIM(hpc.author_name), 1, 9)
                   ELSE
                      SUBSTR(TRIM(hpc.author_name), INSTR(TRIM(hpc.author_name), ' ', -1) + 1, 9)
                   END short_author
               ,hpp.age_from
               ,hpp.age_to
               ,hpp.grade_from
         ...
         SELECT hpp.rowid hpp_rowid
          ...
          WHERE hpc.isbn13 = hpp.isbn13
            ...
            AND (hpp.shipping_schedule IS NOT NULL
                 ...
                 OR hpp.embargo_level IS NOT NULL
   ...
   BEGIN
      ...
      -- Process records to be inserted into FBDI table EGP_SYSTEM_ITEMS_INTERFACE
      BEGIN
      ...
      -- Process records to be inserted into FBDI table EGO_ITEM_INTF_EFF_B with Attribute Group Code = Alternate Item
      BEGIN
      ...
      -- Process records to be inserted into FBDI table EGO_ITEM_INTF_EFF_B with Attribute Group Code = BOM
      BEGIN
            ...
            EXCEPTION
               WHEN le_insert_exception THEN
                  ...
                  FOR i IN 1 .. SQL%BULK_EXCEPTIONS.COUNT LOOP
                     ...
                     BEGIN
                        UPDATE hbg_pim_pam
                           SET error_text = 'Error when loading FBDI table - ' || lv_aux2
                         WHERE ROWID = lv_aux1;
                     EXCEPTION
                        WHEN OTHERS THEN
                           log_p('ERROR'
      ...
      -- Process records to be inserted into FBDI table EGO_ITEM_INTF_EFF_B with Attribute Group Code = By Line
      BEGIN
         ...
         LOOP
            ...
            EXCEPTION
               WHEN le_insert_exception THEN
                  --
                  FOR i IN 1 .. SQL%BULK_EXCEPTIONS.COUNT LOOP
                     lv_aux1 := lv_ego_item_intf_eff_b_by_line_tb(SQL%BULK_EXCEPTIONS(i).ERROR_INDEX).hpp_rowid;
                     lv_aux2 := SQLERRM(-SQL%BULK_EXCEPTIONS(i).ERROR_CODE);
                     --
                     log_p('ERROR'
                     ...
                     BEGIN
                        UPDATE hbg_pim_cpm
                           SET error_text = 'Error when loading EFF FBDI table (context = By Line) - ' || lv_aux2
                         WHERE ROWID = lv_aux1;
                     EXCEPTION
                        WHEN OTHERS THEN
                           log_p('ERROR'
      ...
      -- Process records to be inserted into FBDI table EGO_ITEM_INTF_EFF_B with Attribute Group Code = Carton
      BEGIN
      ...
      -- Process records to be inserted into FBDI table EGO_ITEM_INTF_EFF_B with Attribute Group Code = Configuration
      BEGIN
      ...
      -- Process records to be inserted into FBDI table EGO_ITEM_INTF_EFF_B with Attribute Group Code = Content
      BEGIN
      ...
      -- Process records to be inserted into FBDI table EGO_ITEM_INTF_EFF_B with Attribute Group Code = Contributor
      BEGIN
      ...
      -- Process records to be inserted into FBDI table EGO_ITEM_INTF_EFF_B with Attribute Group Code = Discount Group
      BEGIN
      ...
      -- Process records to be inserted into FBDI table EGO_ITEM_INTF_EFF_B with Attribute Group Code = Distribution Edition Code
      BEGIN
         ...
         LOOP
            ...
            EXCEPTION
               WHEN le_insert_exception THEN
                  ...
                  lv_breakpoints := TRIM(lv_breakpoints || ' / ' || lv_debug);
                  lv_error_msg := TRIM(lv_error_msg || ' ERROR when inserting records into [HBG_EGO_ITEM_INTF_EFF_B]'
                                  || ' Attribute Group Code = Distribution Edition Code');
               WHEN OTHERS THEN
                  lv_breakpoints := TRIM(lv_breakpoints || ' / ' || lv_debug);
                  lv_error_msg := TRIM(lv_error_msg || ' GENERAL ERROR when inserting records into [HBG_EGO_ITEM_INTF'
                                  || '_EFF_B] Attribute Group Code = Distribution Edition Code - ' || SQLERRM);
            END;
         END LOOP;
         --
      ...
      EXCEPTION
         WHEN OTHERS THEN
            ...
            lv_breakpoints := TRIM(lv_breakpoints || ' / ' || lv_debug);
            lv_error_msg := TRIM(lv_error_msg || ' GENERAL EXCEPTION when loading records into [HBG_EGO_ITEM_INTF_EFF_B -'
                            || ' Distribution Edition Code]: ' || SQLERRM);
      END;

      -- Process records to be inserted into FBDI table EGO_ITEM_INTF_EFF_B with Attribute Group Code = Estimated Release
      BEGIN
      ...
      -- Process records to be inserted into FBDI table EGO_ITEM_INTF_EFF_B with Attribute Group Code = Family Code
      BEGIN
      ...
      -- Process records to be inserted into FBDI table EGO_ITEM_INTF_EFF_B with Attribute Group Code = General
      BEGIN
      ...
      -- Process records to be inserted into FBDI table EGO_ITEM_INTF_EFF_B with Attribute Group Code = General BISAC
      BEGIN
      ...
      -- Process records to be inserted into FBDI table EGO_ITEM_INTF_EFF_B with Attribute Group Code = Hot Title Indicator
      BEGIN
      ...
      -- Process records to be inserted into FBDI table EGO_ITEM_INTF_EFF_B with Attribute Group Code = Status and Release
      BEGIN
   ...
   BEGIN
      ...
      dbms_scheduler.create_job(job_name   => 'HBG_LOAD_ITEMS_INFO_INTO_ORACLE_PIM_' || TO_CHAR(p_hbg_process_id)
                               ,job_type   => 'PLSQL_BLOCK'
                               ,job_action => 'BEGIN
                                                  HBG_PIM_INTEGRATION_PKG.MAIN_P(' || p_hbg_process_id
                                               || ',' || p_oic_instance_id || ',' || lv_control_id_step_0 || '); END;'
                               ,enabled    => TRUE
                               ,auto_drop  => TRUE
                               ,comments   => 'HBG Load Items Information into Oracle PIM - HBG Process ID = '
                                              || TO_CHAR(p_hbg_process_id) || ' - OIC Instance ID = '
                                              || TO_CHAR(p_oic_instance_id) || ' - Control ID Step 0 = '

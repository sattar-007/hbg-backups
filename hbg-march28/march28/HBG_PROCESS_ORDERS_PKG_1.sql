...
-----------------------------------------------------------------------------------------------------------------------------------------------
	...
	SELECT  
		...
		LPAD(GIS_HEADER_UID,p_headers_count,'0') AS GIS_HEADER_UID,
		LPAD(LINE_SEQUENCE,p_line_seq_count,'0') as LINE_SEQUENCE,
		LPAD(row_number() over( partition by LINE_SEQUENCE,
											purchase_order_no,
											stage_account_no 
								order by stage_account_no, 
											purchase_order_no, 
											LINE_SEQUENCE),p_line_count,'0')  as ROW_NO,
		EAN,
				...
				EXCEPTION 
					...
					-------------------------------------------------------------------------------*/
						...
						l_update_errmsg := UPDATE_ERROR_MSG	(	l_errbuf, 
																l_doo_header_all_interface_tb(i).STAGE_ACCOUNT_NO, 
			...
			LOOP
				...
				EXCEPTION 
					...
					-------------------------------------------------------------------------------*/
						...
						l_update_errmsg := UPDATE_ERROR_MSG	(	l_errbuf, 
																l_doo_hdr_effs_all_interface_tb(i).STAGE_ACCOUNT_NO, 
			...
			LOOP
				...
				EXCEPTION 
					...
					-------------------------------------------------------------------------------*/
						...
						l_update_errmsg := UPDATE_ERROR_MSG	(	l_errbuf, 
																l_doo_lines_all_interface_tb(i).STAGE_ACCOUNT_NO, 
			...
			-------------------------------------------------------------------------------*/				
				...
				EXCEPTION 
					...
					-------------------------------------------------------------------------------*/
						...
						l_update_errmsg := UPDATE_ERROR_MSG	(	l_errbuf, 
																l_doo_lines_effs_all_interface_tb(i).STAGE_ACCOUNT_NO, 
			...
			-------------------------------------------------------------------------------*/
				...
				EXCEPTION 
					...
					-------------------------------------------------------------------------------*/
						...
						l_update_errmsg := UPDATE_ERROR_MSG	(	l_errbuf, 
																l_doo_addresses_all_interface_tb(i).SOURCE_ACCOUNT_NO, 
...
-----------------------------------------------------------------------------------------------------------------------------------------------
	...
	BEGIN 
		...
		LOOP
			...
			SELECT LENGTH(MAX(GIS_HEADER_UID))
			...
			SELECT 	LENGTH(MAX(COUNT_LINES)) 
					INTO l_no_same_line
			FROM (SELECT COUNT(1) AS COUNT_LINES, LINE_SEQUENCE 
					FROM HBG_STERLING_DETAIL 
					WHERE SBI_UUID = l_gisid_rec.SBI_UUID
					group by LINE_SEQUENCE);

            SELECT  LENGTH(MAX(LINE_SEQUENCE)) 
		...
		-------------------------------------------------------------------------------------------------------*/
			...
			*/-------------------------------------------------------------
            ...
            IF l_fbdi_retcode = 1 THEN
				...
				l_update_errmsg := UPDATE_ERROR_MSG	(l_errbuf, NULL,NULL, l_gisid_rec.SBI_UUID, 'NEW');
				errbuf := l_errbuf;
				RAISE file_exception;
...
-----------------------------------------------------------------------------------------------------------------------------------------------  
 PROCEDURE VALIDATE_STERLING_DATA
	...
	SELECT DISTINCT
		...
		hef1.ATTRIBUTE_CHAR9		AS SHIPTO_COUNTRY,   	      
...
------------------------------------------------------------------------------------------------------*/
	...
	/*=========================================================================*/
    /*              SEND ADDRESS ERRORS TO OTHER TABLES                         */
    /*=========================================================================*/	

        dbms_output.put_line('SEND ADDRESS ERRORS TO OTHER TABLES  - ' || systimestamp);
        UPDATE HBG_PROCESS_ORDERS_HEADERS_FBDI hdr
        SET STATUS = 'ERROR'

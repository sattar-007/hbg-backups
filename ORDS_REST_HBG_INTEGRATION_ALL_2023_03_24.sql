...
-- Schema: HBG_INTEGRATION  Date: Fri Mar 24 07:14:54 2023 
--
...
BEGIN
    ...
    , p_flag_code => :flag_code,
                                               ...
                                               , p_include_claim => :include_claim, p_include_unapp_receipts => :include_receipts, p_loggedin_user => :loggedin_user,
                                               p_held_by_others => :held_by_othrs_data, x_transactions => transactions, p_ret_status => ret_status, p_error_msg => error_msg
                                               );
...
'select OWNER_ID,OWNER_NAME	 from XXHBG_FAMILY_CODE_OWNER where OWNER_CODE = :OWNER_CODE');
  ...
  ORDS.DEFINE_HANDLER(
      ...
      p_comments       => NULL,
      p_source         => 
'Select Account_number from hz_cust_Accounts hca where
...
'select deptno from emp');
  ...
  ORDS.DEFINE_HANDLER(
      ...
      p_comments       => NULL,
      p_source         => 
'select item_number from EGP_SYSTEM_ITEMS_B where item_number like :p_item_num || ''%''');
...
'SELECT account_number, account_name FROM hz_cust_accounts where :p_account_number is null or account_number = :p_account_number order BY account_number');
  ...
  ORDS.DEFINE_HANDLER(
      ...
      p_comments       => NULL,
      p_source         => 
'select DISTINCT party_name from hz_parties where party_type = ''ORGANIZATION''');
...
WHERE
    ...
    AND upper(nvl(a.ship_to_site, ''#NULL'')) LIKE upper(nvl(:p_shipto, nvl(a.ship_to_site, ''#NULL'')))
...
,SALES_FORCE_ID
,ACCOUNT_NUMBER
,ACCOUNT_NAME from XXHBG_UNASSIGNED_SALES_FORCE_TBL

 where         1 = 1

    AND (
 ( :SALES_FORCE_NUMBER IS NULL
            OR SALES_FORCE_NUMBER = :SALES_FORCE_NUMBER )
...
,SF_ACCOUNT_ID from xxhbg_assign_account_type_to_sales_force_tbl
 ...
 ( :SF_ACCOUNT_ID IS NULL
            OR SF_ACCOUNT_ID = :SF_ACCOUNT_ID )
...
,DF_DISTRIBUTION_ID
,ACCOUNT_NUMBER
,ACCOUNT_NAME from xxhbg_default_distribution_channel_tbl
WHERE
        1 = 1

    AND (
 ( :SALES_FORCE IS NULL
            OR SALES_FORCE = :SALES_FORCE )

          AND ( :DISTRIBUTION_CHANNEL IS NULL
                OR DISTRIBUTION_CHANNEL = :DISTRIBUTION_CHANNEL ) 
          AND ( :P_STATUS IS NULL
                OR STATUS = :P_STATUS )
  )
...
WHERE
 ...
 ( :P_SALES_FORCE_NUMBER IS NULL
            OR SALES_FORCE_NUMBER = :P_SALES_FORCE_NUMBER )

          AND ( :P_SALES_FORCE_NAME IS NULL
                OR SALES_FORCE_NAME = :P_SALES_FORCE_NAME )
...
1=1 
AND 
   ( :SALES_FORCE IS NULL  OR SALES_FORCE = :SALES_' || 'FORCE )
   AND 
   ( :OVERRIDE IS NULL  OR OVERRIDE = :OVERRIDE )
 AND ( :P_STATUS IS NULL
                OR STATUS = :P_STATUS )
...
,STATUS
,SALES_FORCE_ID
,SF_ACCT_TYPE_ID from XXHBG_SALES_FORCE_MAPPING_TBL
   WHERE
   ( :sales_force_number IS NULL  OR sales_force = :sales_force_number )
AND ( :inv_item_id IS NULL  OR owner||reporting_group||category_1||category_2 = (select ATTRIBUTE_CHAR1||ATTRIBUTE_CHAR2||ATTRIBUTE_CHAR3||ATTRIBUTE_CHAR4
  ...
  and ORGANIZATION_ID=(select ORGANIZATION_ID from inv' || '_org_parameters where ORGANIZATION_CODE=''ITEM_MASTER'') )

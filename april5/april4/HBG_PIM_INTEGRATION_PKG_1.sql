...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "HBG_INTEGRATION"."HBG_PIM_INTEGRATION_PKG" IS
   ...
   BEGIN
      ...
      BEGIN
         ...
         ON (heiieb.item_number = hpp.isbn13
             ...
             AND heiieb.context_code = 'Distribution Edition Code'
             AND hpp.hbg_process_id = p_hbg_process_id)
         WHEN MATCHED THEN
            UPDATE
               SET hpp.status = CASE
                                   ...
                                   ELSE
                                      heiieb.status
                                END
                  ,hpp.error_text = TRIM(hpp.error_text || ' ' || heiieb.error_text);
         --
         COMMIT;
      EXCEPTION
         WHEN OTHERS THEN
            ...
            lv_error_msg := 'ERROR when merging STATUS and ERROR_TEXT into table [HBG_PIM_PEM] from [' ||
                            'HBG_EGO_ITEM_INTF_EFF_B - Distribution Edition Code]. ' || SQLERRM;
            RAISE ge_custom_exception;
      END;
            ...
            UPDATE
               SET hpp.status = CASE
                                   ...
                                   ELSE
                                      heici.status
                                END
                  ,hpp.error_text = TRIM(hpp.error_text || ' ' || heici.error_text);
         --
         COMMIT;
      EXCEPTION
         WHEN OTHERS THEN
      ...
      BEGIN
         ...
         ON (heiieb.item_number = hpp.isbn13
             AND heiieb.batch_id = hpp.hbg_process_id
             AND heiieb.attribute_char1 = hpp.genbisac_code
      ...
      BEGIN
         ...
         ON (heiieb.item_number = hpp.isbn13
             ...
             --AND (heiieb.attribute_date2 = hpp.on_sale_date OR hpp.on_sale_date IS NULL)
            ...
            UPDATE
               SET hpc.status = CASE
                                   ...
                                   ELSE
                                      heiieb.status
                                END
                  ,hpc.error_text = TRIM(hpc.error_text || ' ' || heiieb.error_text);
         --
         COMMIT;
      EXCEPTION
         WHEN OTHERS THEN
         ...
         UPDATE hbg_pim_cpm hpc
            SET hpc.status = CASE
               ...
               ,hpc.error_text = TRIM(hpc.error_text || ' ' ||
                                      (SELECT LISTAGG(hesii.error_text, ' ' ON OVERFLOW TRUNCATE '!!!') WITHIN GROUP (ORDER BY hesii.organization_code)
                                         FROM hbg_egp_system_items_interface hesii
                                        WHERE hesii.item_number = hpc.isbn13
                                          AND hesii.batch_id = hpc.hbg_process_id))
          WHERE hpc.hbg_process_id = p_hbg_process_id;
         --
         COMMIT;
      EXCEPTION
         WHEN OTHERS THEN
      ...
      IF lv_keep_data_in_fbdi_tables = 'N' THEN
         ...
         BEGIN
            ...
            DELETE
             ...
             WHERE batch_id = p_hbg_process_id;
   ...
   --
   -- -----------------------------------------------------------------------------------------------------------------
   PROCEDURE update_fbdi_tables_status_to_error_p(p_hbg_process_id  IN NUMBER) IS
   ...
   --
   -- -----------------------------------------------------------------------------------------------------------------
   PROCEDURE update_process_tables_status_to_error_p(p_hbg_process_id  IN NUMBER) IS
   ...
   BEGIN
      ...
      lv_debug := 'VALIDATE_OWNER_IN_HBG_PIM_CPM';
      BEGIN
         UPDATE hbg_pim_cpm hpc
            SET hpc.status = 'ERROR'
      ...
      EXCEPTION
         WHEN OTHERS THEN
            ROLLBACK;
            log_p('ERROR'
                 ,'ERROR when updating HBG_PIM_CPM status to ERROR because of [OWNER_CODE IS NULL] - ' || SQLERRM);
            RAISE ge_custom_exception;
      END;
   ...
   BEGIN
      ...
      -- Process records to be inserted into FBDI table HBG_EGO_ITEM_INTF_EFF_B with Attribute Group Code = Distribution Edition Code
      BEGIN

...
--------------------------------------------------------
  ...
  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "HBG_INTEGRATION"."HBG_PRE_RESERVATIONS_PKG" AS
    ...
    BEGIN
        ...
        IF ( p_item_code IS NULL OR p_reservation_type IS NULL OR p_requested_quantity IS NULL OR p_lot_number IS NULL) THEN
            p_return_status := 'Please enter the mandatory feilds Item Code, Reservation Type, Lot Number & Requested Quantity';
        ELSIF (
            l_count = 0
        ...
        ELSIF l_item_count > 0  THEN --and l_check_count < 1
...
--            ELSE
                p_return_status := 'SUCCESS';
--            END
        ELSIF l_item_count2 > 0  THEN --and l_check_count < 1
            p_return_status := 'Item '
                               || p_item_code
                               || ' with subinventory '
                               || p_subinventory

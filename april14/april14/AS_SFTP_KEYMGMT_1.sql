--------------------------------------------------------
--  DDL for Package Body AS_SFTP_KEYMGMT
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "HBG_INTEGRATION"."AS_SFTP_KEYMGMT" AS
    --
    ...
    BEGIN
        IF i_trust_server THEN
            as_sftp.open_connection(i_host => i_host, i_trust_server => TRUE, i_port => i_port);

            -- not sure required to close and reopen, but that is how Anton's example works
            as_sftp.close_connection;
        END IF;
        as_sftp.open_connection(i_host => i_host, i_port => i_port);
